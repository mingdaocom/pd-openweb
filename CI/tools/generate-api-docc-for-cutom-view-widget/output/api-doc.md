...
args.worksheetId  {string}  工作表id  
args.dec  {string}  详细说明  
args.resume  {string}  摘要  
args.remark  {string}  工作表描述（新加的 支持AI生成）  
...
args.discussId  {string}  讨论ID  
args.langType  {integer}  //语言类型  
args.controlId  {string}  预览附件的控件id  
args.fileId  {string}  当前预览的附件id  

```js
import { apis } from "mdye";

apis.worksheet.getAttachmentList(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.worksheet.chooseRelationRows(args)
  .then(res => {
    console.log(res);
  });
```
...
args.rowIds  {array}    
args.btnType  {integer}  -1:全部 0：按钮 1：AI Action  
...
args.name  {string}    
args.worksheetId  {string}    
args.btnType  {integer}  按钮类型 1：AI Action  
...
args.worksheetId  {string}  工作表id  
args.relationWorksheetId  {string}    
args.controlIds  {array}  批量工作表id  
args.getRelationSearch  {boolean}  获取查下记录控件  
args.resultType  {integer}  获取类型 0或者1：常规 2：简易模式 3:严格鉴权  
...
apis.worksheet.getControlsByIds(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.worksheet.getPrintDetailList(args)
  .then(res => {
    console.log(res);
  });
```
...
args.viewIds  {array}    
args.displayFlowChart  {integer}  显示流转图 （0 = 显示，1 = 不显示）  
...
apis.worksheet.handleAIRequest(args)
  .then(res => {
    console.log(res);
  });
```
...
args.recordsNotFound  {integer}  0 = 赋空值 1 = 保留原值  
args.moreSort  {array}  排序  
args.queryCount  {integer}  查询条数  
args.resultType  {integer}  结果类型 0=查询到记录，1=仅查询到一条记录，2=查询到多条记录，3=未查询到记录  
args.eventType  {integer}  0 = 常规字段默认值，1 = 表单事件，2 = 业务规则设置字段默认值  
...
args.previewed  {boolean}  导出附件  
args.exportId  {boolean}  导出id  
args.exportJob  {boolean}  导出工号  
args.speed  {boolean}  加速导出  
...
args.rowId  {string}  记录id  
args.viewId  {string}  视图id  
args.checkView  {boolean}  是否校验视图权限  
args.setFollow  {boolean}  是否关注  

```js
import { apis } from "mdye";

apis.worksheet.setFollow(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.worksheet.getFollower(args)
  .then(res => {
    console.log(res);
  });
```
...
args.isFilterChatBot  {boolean}  是否过滤对话机器人  

```js
import { apis } from "mdye";

apis.appManagement.getAppItems(args)
  .then(res => {
    console.log(res);
  });
```
...
args.resume  {string}  摘要  
args.sourceType  {integer}  来源 1 = mingo修改  
...
args.name  {string}  名称  
args.remark  {string}  描述  
args.icon  {string}  Logo  
args.type  {integer}  类型 0=工作表 1=自定义页面,3 = 对话机器人  
args.createType  {integer}  创建类型（创建自定义页面得时候需要传）0-表示普通 1-表示外部链接  
args.urlTemplate  {string}  链接  
args.configuration  {object}  链接配置  
args.sourceType  {integer}  创建来源类型（默认0，1= ai创建）  
args.prompt  {string}  创建机器人提示词  
...
args.appId  {string}  应用id  
args.id  {string}  应用项id  
args.remark  {string}  描述  
args.desc  {string}  详细说明  
args.resume  {string}  说明  

```js
import { apis } from "mdye";

apis.appManagement.updateChatBotDesc(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.appManagement.copyChatBot(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.appManagement.getExportsByProject(args)
  .then(res => {
    console.log(res);
  });
```
...
args.sourceId  {string}  分享来源id （页面id，图标id等）  
args.sourceType  {integer}  分享类型  21 =自定义页面，31 = 图表，70 = 对话机器人，71 = 对话机器人对话历史  
...
args.projectId  {string}  组织id  
args.toViewUrl  {string}  【前往查看】 Url 地址  

```js
import { apis } from "mdye";

apis.appManagement.isFirstInactiveUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.appManagement.queryInactiveUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  组织id  
args.dayRange  {integer}  天数范围  
args.pageIndex  {integer}  当前页  
args.pageSize  {integer}  页大小  
args.orderBy  {integer}  排序 MD.Entity.Mongo.Apk.InactiveUserSort  

```js
import { apis } from "mdye";

apis.appManagement.pagedInactiveUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
args.modelType  {integer}  覆盖类型， 0 = 默认，1 = 全量  
...
args.pageIndex  {integer}    
args.pageSize  {integer}    
args.status  {integer}  不筛选就不传，状态 0=已完成， 1=进行中， 2=失败  

```js
import { apis } from "mdye";

apis.appManagement.getUpgradeLogsByProject(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.appManagement.loadRelationLangData(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.appManagement.getRelationAppLangDetail(args)
  .then(res => {
    console.log(res);
  });
```
...
## getItemDetailByAppId

批量获取应用下应用项信息

### 参数

args.appId  {string}  应用id（应用预览鉴权)  
args.itemIds  {array}  应用项id  

```js
import { apis } from "mdye";

apis.homeApp.getItemDetailByAppId(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.homeApp.getAppLangInfo(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.instanceVersion.batch2(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.instanceVersion.pass2(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.instanceVersion.overrule2(args)
  .then(res => {
    console.log(res);
  });
```
...
args.chatbotId  {string}  *null  

```js
import { apis } from "mdye";

apis.process.getChatbotConfig(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.process.saveChatbotConfig(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.loadProvince(args)
  .then(res => {
    console.log(res);
  });
```
...
args.id  {string}  地区id  
args.textSplit  {string}  分割字符  

```js
import { apis } from "mdye";

apis.fixedData.loadCityCountyById(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.getCityByID(args)
  .then(res => {
    console.log(res);
  });
```
...
args.parentId  {string}  上级ID  
args.keywords  {string}  关键词  
args.layer  {integer}  层级 -1：不限制  
args.textSplit  {string}  分割字符  
args.isLast  {boolean}  是否最后一级  
args.langType  {integer}  语言类型  
args.projectId  {string}  组织Id  
args.isGetCounty  {boolean}  返回path是否包含国家  
args.isReturnWithPhoneCode  {boolean}  返回值是否带上区号  

```js
import { apis } from "mdye";

apis.fixedData.getCitysByParentID(args)
  .then(res => {
    console.log(res);
  });
```
...
args.cityId  {string}  城市ID  
args.textSplit  {string}  分割字符  
args.langType  {integer}  语言类型  
args.projectId  {string}  组织Id  
args.isGetCounty  {boolean}  返回path是否包含国家  
args.isChooseCounty  {boolean}  是否需要返回第一级  

```js
import { apis } from "mdye";

apis.fixedData.getAllPathCitysByID(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.getRegionConfigInfos(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.loadIndustry(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.checkSensitive(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.loadTimeZones(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.loadExtraDatas(args)
  .then(res => {
    console.log(res);
  });
```
...
args.langCodes  {array}    

```js
import { apis } from "mdye";

apis.fixedData.loadLangList(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.fixedData.loadHostKeys(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  网络Id  
args.accountIds  {array}  用户Id集  

```js
import { apis } from "mdye";

apis.user.getUserListByAccountId(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  网络Id  
args.accountId  {string}  用户Id  
args.setAcountPravite  {boolean}  是否设为私密  

```js
import { apis } from "mdye";

apis.user.getUserCard(args)
  .then(res => {
    console.log(res);
  });
```
...
args.onProjectId  {string}  使用的模块所在组织Id（可空），如应用  
args.appId  {string}  所在应用  
args.accountId  {string}  账号Id  
args.refresh  {boolean}  是否刷新读取  

```js
import { apis } from "mdye";

apis.user.getAccountBaseInfo(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.checkAccountSecured(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.getAccountDetail(args)
  .then(res => {
    console.log(res);
  });
```
...
args.search  {integer}  搜索类型  
args.keywords  {string}  关键词  
args.currentProjectId  {string}  当前组织（可不传）  
args.pageSize  {integer}  页大小  

```js
import { apis } from "mdye";

apis.user.getUsersByKeywords(args)
  .then(res => {
    console.log(res);
  });
```
...
args.count  {integer}  页大小（经常协作用户的数量，不包含 prefixAccountIds 和未指定/我自己的数量）  
args.filterAccountIds  {array}  需要排除的帐号  
args.prefixAccountIds  {array}  需要插在前面的帐号，排在未指定和我自己后面  
args.includeUndefinedAndMyself  {boolean}  是否在前面插入未指定和我自己  
args.includeSystemField  {boolean}  是否包含系统预设账户  
args.oftenMetionedType  {integer}  最常协作类型：-1：遵循用户的设置；0：系统推荐；1：用户自定义  
args.projectId  {string}  当前网络id  

```js
import { apis } from "mdye";

apis.user.getOftenMetionedUser(args)
  .then(res => {
    console.log(res);
  });
```
...
args.pageIndex  {integer}  页码  
args.pageSize  {integer}  页大小  
args.keywords  {string}  关键司  
args.projectId  {string}  网络Id  
args.firstLetter  {string}  过滤的首字母  
args.filterAccountIds  {array}  过滤的需要排除的帐号  
args.prefixAccountIds  {array}  需要插在前面的帐号，排在未指定和我自己后面  
args.includeUndefinedAndMyself  {boolean}  是否在前面插入未指定和我自己  
args.includeMyself  {boolean}  是否在前面插入我自己  
args.includeSystemField  {boolean}  是否包含系统预设账户  
args.appointedAccountIds  {array}  指定的账户列表  
args.appointedDepartmentIds  {array}  指定的部门列表  
args.appointedOrganizeIds  {array}  指定的组织角色列表  
args.takeTotalCount  {boolean}  是否获取 总数  

```js
import { apis } from "mdye";

apis.user.getContactUserList(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.getProjectResignedUserList(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.getProjectContactUserListByApp(args)
  .then(res => {
    console.log(res);
  });
```
...
args.pageSize  {integer}  页大小  
args.keywords  {string}  关键词  
args.firstLetter  {string}  首字母  
args.filterAccountIds  {array}  过滤哪些账号  

```js
import { apis } from "mdye";

apis.user.getLeaveUserList(args)
  .then(res => {
    console.log(res);
  });
```
...
args.accountIds  {array}  账户Ids  
args.projectId  {string}  网络Id  

```js
import { apis } from "mdye";

apis.user.sendNotice(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.validateUserIsProjectAdmin(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.validateUserHaveFriend(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.pagedNormalUserList(args)
  .then(res => {
    console.log(res);
  });
```
...
args.pageSize  {integer}  pageSize  
args.keywords  {string}  关键词  
args.keywordsType  {integer}  关键词查询 类型（多字段[默认]=0、名称=1、工号=2、手机号=3、邮箱=4）  
args.sort  {integer}  排序字段（默认 0=离职时间 倒序）  

```js
import { apis } from "mdye";

apis.user.pagedRemovedUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  组织Id  
args.userContact  {string}  用户的联系方式  
args.accountId  {string}  用户的账号Id  
args.departmentId  {string}  部门Id  

```js
import { apis } from "mdye";

apis.user.getUserOrgState(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.getProjectContactUserList(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.getApprovalUser(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  网络Id  
args.accountId  {string}  账号Id  
args.companyName  {string}  公司名  
args.jobNumber  {string}  Job号  
args.contactPhone  {string}  联系号码  
args.workSiteId  {string}  工作地点  
args.useMultiJobs  {boolean}  使用 多职位（false 则 未使用；true 代表使用，则 只使用新 部门职位关系属性 入库）  
args.departmentIds  {array}  部门（第一个为主部门）  
args.jobIds  {array}  职位ids  
args.departmentJobIdMaps  {array}  任职信息（第一个 代表主部门）  
args.orgRoleIds  {array}  组织角色ids  

```js
import { apis } from "mdye";

apis.user.updateUserCard(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  网络Id  
args.accountId  {string}  账户Id  

```js
import { apis } from "mdye";

apis.user.removeUser(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  网络Id  
args.accountIds  {array}  账户Id  

```js
import { apis } from "mdye";

apis.user.removeUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  网络Id  
args.accountIds  {array}  账户Ids  
args.departmentIds  {array}  部门ids 第一个为主部门  

```js
import { apis } from "mdye";

apis.user.updateDepartmentForUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.updateJobForUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
args.projectId  {string}  网络Id  
args.accountIds  {array}  账户Ids  
args.worksiteId  {string}  工作地点id  

```js
import { apis } from "mdye";

apis.user.updateWorkSiteForUsers(args)
  .then(res => {
    console.log(res);
  });
```
...
args.accountId  {string}  账户Id  
args.departmentId  {string}  部门Id  
args.departmentIds  {array}  新部门Ids（第一个为主部门）  
args.workSiteId  {string}  工作地点  
args.jobId  {string}  职位  
args.jobIds  {array}  职位Ids  
args.orgRoleIds  {array}  组织角色Id  
args.jobNumber  {string}  工号  
args.contactPhone  {string}  联系号码  
args.useMultiJobs  {boolean}  使用 多职位（false 则 未使用；true 代表使用，则 只使用新 部门职位关系属性 入库）  
args.departmentJobIdMaps  {array}  任职信息（第一个 代表主部门）  
...
args.projectId  {string}  网络Id  
args.accountId  {string}  账户Id  
args.refuseMessage  {string}  拒绝消息  

```js
import { apis } from "mdye";

apis.user.refuseUserJoin(args)
  .then(res => {
    console.log(res);
  });
```
...
apis.user.refuseUsersJoin(args)
  .then(res => {
    console.log(res);
  });
```
