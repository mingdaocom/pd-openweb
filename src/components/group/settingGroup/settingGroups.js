...
SettingGroup.DEFAULT = {
  ...
  groupAction: { // groupAction callback type
...
$.extend(SettingGroup.prototype, {
  ...
  showDialog: function () {
    ...
    groupController.getGroupInfo({
      ...
      if (result && result.status === 1) {
        ...
        // render dialog
        _this.dialog.content(doT.template(require('./tpl/main.html'))(options));
        // get DOM
        _this.getContent();
        // bind event
        _this.initEvent();
        ...
        if (_this.options.isPost) {
          ...
          _this.$groupInfo.removeClass('Hidden').html(LoadDiv())
  ...
  bindHeadEvent: function () {
    ...
    $(document).off('click.groupSetting scroll.groupSetting').on('click.groupSetting scroll.groupSetting', function () {
    ...
    if (options.isAdmin) {
      ...
      $groupInfo.on('blur', '.groupTextBox', function () {
        ...
        if ($this.is('.groupName')) {
          ...
          _this.updateGroupDesc(val).then(function () {
            $this.val(val).data('content', val);
            options.isRefresh.info = true;
  ...
  bindGroupMemberEvent: function () {
    ...
    $(document).off('click.groupSettingUserOp').on('click.groupSettingUserOp', function (event) {
  ...
  bindGroupSettingEvent: function () {
    ...
    $groupSettings.on('click', '.officialDepSelect', function () {
      ...
      if (options.projectId) {
        require([
          'src/components/dialogSelectMapGroupDepart/dialogSelectMapGroupDepart'
        ], function () {
  ...
  bindCommEvent: function () {
    ...
    _this.$container.on('mouseover', 'img[data-accountid]', function () {
      ...
      var opHtml = !options.isPost && accountId !== md.global.Account.accountId && options.isAdmin ? '<div class="removeUser TxtCenter Hand" data-accountid="' + accountId + '">' + tips.remove + '</div>' : '';
      $(this).mdBusinessCard({
        reset: true,
        secretType: 1,
        accountId: accountId,
  ...
  toggleGroupTab: function (type) {
    ...
    _this.$tabList.find('.commItem').eq(index).addClass('activation')
      .siblings().removeClass('activation');

    _this.$content.children().eq(index).removeClass('Hidden')
      .siblings().addClass('Hidden');
  ...
  quickInviteEvent: function () {
    ...
    require(["dialogSelectUser"], function (dialogSelectUser) {
      // add member
  ...
  loadGroupInfo: function () {
    ...
    $groupInfo.removeClass('Hidden').html(LoadDiv())
      .siblings().addClass('Hidden');
    // 是否重新获取 新的数据
  ...
  renderGroupInfo: function ($box) {
    ...
    var tpl = doT.template(require('./tpl/groupInfo.html'))(group);
    $box.html(tpl);
  ...
  loadGroupMember: function () {
    ...
    $groupMember.removeClass('Hidden').html(LoadDiv())
      .siblings().addClass('Hidden');
    // 是否重新获取 新的数据
  ...
  renderGroupMember: function ($box, isAppend) {
    ...
    $box.html(doT.template(require('./tpl/groupUser.html'))(renderData));
  ...
  loadGroupSettings: function () {
    ...
    $groupSettings.removeClass('Hidden').html(LoadDiv())
      .siblings().addClass('Hidden');
    // 是否重新获取 新的数据
  ...
  renderGroupSettings: function ($box) {
    ...
    var tpl = doT.template(require('./tpl/groupSettings.html'))(options.data);
    $box.html(tpl);
  ...
  updateGroupToPost: function () {
    ...
    var projectId = '';
    var dict = {};
    var content = '<p class="mTop15 Gray_6">' + tips.convertProject + '<input type="hidden" id="selectProject"></p>' +
      '<p class="mTop15 Gray_6">' + tips.convertTip + '</p>';
    ...
    require(
      ['mdDialog', 'mdFunction', 'md.select'],
      function (dialogLayer, mdFunction) {
        var dialog = dialogLayer.index({
          ...
          container: {
            ...
            content: content,
            yesFn: function () {
              groupController.updateGroupToPost({
                groupId: options.groupID,
                projectId: projectId,
              }).then(function (result) {
                if (result) {
          ...
          readyFn: function () {
            ...
            $dialog.find('#selectProject').MDSelect({
              ...
              onChange: function (id, name) {
                projectId = id;
                mdFunction.expireDialogAsync(projectId)
                  .then(function () {
                    dialog.enable();
                  }, function () {
                    dialog.disable();
  ...
  updateGroupApprove: function ($switch) {
    ...
    groupController.updateGroupApproval({
      groupId: options.groupID,
  ...
  updateGroupList: function ($switch) {
    ...
    groupController.updateGroupHidden({
      groupId: options.groupID,
      isHidden: isChecked,
    }).then(function (result) {
      if (result) {
  ...
  updateGroupInviteAuth: function ($switch) {
    ...
    groupController.updateGroupForbidInvite({
      groupId: options.groupID,
  ...
  updateGroupChatAuth: function ($switch) {
    ...
    groupController.updateGroupForbidSpeak({
      groupId: options.groupID,
  ...
  updateGroupDepartment: function (deptId, deptName) {
    ...
    groupController.updateGroupVerified({
      groupId: options.groupID,
      isVerified: !isClose,
  ...
  updateUserCallback: function (accountId, type) {
    ...
    if (type === 'pass') {
      $op.find('.settingMemberRole').data('type', 'member')
        .find('.roleName')
        .text(tips.roleMember);
    } else if (type === 'admin') {
      $op.find('.settingMemberRole').data('type', 'admin')
        .find('.roleName')
        .text(tips.roleAdmin);
    } else if (type === 'member') {
      $op.find('.settingMemberRole').data('type', 'member')
  ...
  addMembers: function (userArray) {
    ...
    invitationController.inviteUser({
      ...
      require(['mdFunction'], function (mdFunction) {
        var formatedData = mdFunction.existAccountHint(result);
        if (formatedData.accountInfos.length) {
          _this.opSuccess(options.groupAction['ADD_MEMBERS'], {
            groupId: options.groupID,
  ...
  inviteMembers: function (accounts, callbackInviteResult) {
    ...
    invitationController.inviteUser({
      ...
      require(['mdFunction'], function (mdFunction) {
        var formatedData = mdFunction.existAccountHint(result, callbackInviteResult);
        if (formatedData.accountInfos.length) {
  ...
  removeUser: function (accountId, type) {
    ...
    groupController[func]({
      ...
      if (result) {
        _this.opSuccess(options.groupAction['REMOVE_MEMBER'], {
          groupId: options.groupID,
          accountId: accountId,
        }, tip);
        // 回调
  ...
  deleteGroup: function (groupIds) {
    ...
    new Confirm({
      content: options.isPost ? tips.confirmDelGroupText : tips.confirmDelText,
  ...
  closeGroup: function (groupIds, groupName) {
    ...
    new Confirm({
      content: tips.confirmCloseText1 + "<br/>" + tips.confirmCloseText2,
      ...
      groupController.closeGroup({
        ...
        if (result) {
          _this.dialog.closeDialog();
          _this.opSuccess(options.groupAction['CLOSE_GROUP'], {
            groupId: groupIds,
  ...
  exitGroup: function (groupId, groupName) {
    ...
    new Confirm({
      content: options.isPost ? tips.confirmExitTextPostGroup : tips.confirmExitTextGroup,
      ...
      groupController.exitGroup({
        ...
        if (result == ActionResult.Success) {
          ...
          _this.dialog.closeDialog();
          _this.opSuccess(options.groupAction['EXIT_GROUP'], {
            groupId: groupId,
          }, tip);
        } else if (result == ActionResult.OnlyGroupAdmin) {
          _this.opFailed(tips.onlyAdminTip, 3);
