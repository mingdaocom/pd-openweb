...
import { accAdd, accDiv, accMul, accSub, formatNumberThousand, formatStrZero, toFixed } from 'src/util';
...
const Numeric = props => {
  const {
    ...
    dot,
    enumDefault,
    advancedSetting: { datamask, dotformat, showtype, numinterval, numshow, prefix, suffix = props.unit, thousandth },
    triggerCustomEvent,
    value,
  } = props;
  const getEditValue = () => {
  ...
  const getShowValue = () => {
    ...
    return value ? `${prefix || ''}${value}${suffix || ''}` : hint || '';
  };
...
export default memo(Numeric, (prevProps, nextProps) => {
  return _.isEqual(_.pick(prevProps, ['value', 'disabled', 'formDisabled']), _.pick(nextProps, ['value', 'disabled', 'formDisabled']));
});
