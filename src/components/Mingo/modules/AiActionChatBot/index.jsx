...
const MINGO_MAX_WIDTH = 600;
const MINGO_DEFAULT_WIDTH = 420;
...
const MingoContentWrap = styled.div`
  ...
  overflow: hidden;
  background:
    linear-gradient(135deg, rgba(110, 9, 249, 0.08), rgba(55, 29, 255, 0.03), rgba(55, 29, 255, 0.03)),
...
function MingoContent(props, ref) {
  ...
  const [panelWidth, setPanelWidth] = useState(() => {
    const savedWidth = parseInt(localStorage.getItem('AI_ACTION_CHATBOT_WIDTH') || MINGO_DEFAULT_WIDTH, 10);
    return isNaN(savedWidth) ? MINGO_DEFAULT_WIDTH : Math.min(Math.max(savedWidth, MINGO_MIN_WIDTH), MINGO_MAX_WIDTH);
  });
  ...
  const [mode, setMode] = useState(defaultActiveButtonId || isTest ? MODE.CHAT : MODE.BUTTONS);
  const [activeButtonId, setActiveButtonId] = useState(defaultActiveButtonId);
  ...
  const {
    ...
    aiCompletionApi: async (messages, { prevUserMessageId, toolMessageId, abortController }) => {
      return chatbotSSEApi.startProcess_1(
        {
          appId: worksheetId,
  ...
  const handleClearConversation = useCallback(() => {
    ...
    localStorage.removeItem(`aiActionLatestButton-${get(md, 'global.Account.accountId')}-${recordId}`);
  ...
  useEffect(() => {
    ...
    if (mode === MODE.BUTTONS) {
      const latestButtonId = localStorage.getItem(
  ...
  const actualWidth = allowResize ? panelWidth : MINGO_DEFAULT_WIDTH;
  return (
    <MingoContentWrap
      className={cx(className, { isMobile })}
      ref={conRef}
      style={actualWidth && !isMobile ? { width: actualWidth } : {}}
    >
      ...
      {dragMaskVisible && (
        <DragMask
          ...
          value={dragLeft}
          min={window.innerWidth - MINGO_MAX_WIDTH}
          max={window.innerWidth - MINGO_MIN_WIDTH}
      ...
      {mode === MODE.BUTTONS && (
        <TriggerButtons
          ...
          onReloadButtons={onReloadButtons}
        />
      )}
      {mode === MODE.CHAT && (
        <MessageList
          ...
          handleRegenerate={async ({ messageId }) => {
            ...
            setMessages(prev => {
              ...
              handleFetch(newMessages, { prevUserMessageId });
              return newMessages;
            });
          }}
      ...
      {mode === MODE.CHAT && !disabled && !showMessagesOnly && !shareMode && (
