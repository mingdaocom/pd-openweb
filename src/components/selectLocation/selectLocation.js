...
var SelectLocation = function(el, param) {
  ...
  _this.init = function() {
    ...
    $el.on('focus', function(event) {
      ...
      if ($el.next('.locationLayer').length == 0) {
        ...
        // 移除页面上多余的
        $('.locationLayerContainer .locationLayer')
          .not($el.next('.locationLayer'))
          .remove();
        ...
        // 城市地区
        var cityLocationContent = $('<div>')
          .addClass('locationList Hidden')
          .attr('tag', 'city');
        // 区县地区
        var countyLocationContent = $('<div>')
          .addClass('locationList Hidden')
          .attr('tag', 'county');
  ...
  _this.showActiveTag = function(tag) {
    ...
    $locationTabs
      .find('li')
      .removeClass('activeTab ThemeColor3 disbaleTab')
      .addClass('Hand');
    $locationTabs
      .find("li[tag='" + tag + "']")
...
export default (el, param) => {
  return new SelectLocation(el, param);
};
