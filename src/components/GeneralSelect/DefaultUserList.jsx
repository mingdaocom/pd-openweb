...
export default class DefaultUserList extends Component {
  ...
  render() {
    ...
    if ((data.oftenUsers && data.oftenUsers.list && data.oftenUsers.list.length) || (data.users && data.users.list.length)) {
      return (
        <div className="GSlect-defaultUsersContent">
          {data.oftenUsers && data.oftenUsers.list && data.oftenUsers.list.length ? (
            <div>
              ...
              {data.oftenUsers.list.map(user => (
                <User user={user} {...includeUndefinedAndMySelf} onChange={this.props.onChange} checked={this.getChecked(user)} key={'oftenUser' + user.accountId} />
              ))}
            </div>
          ) : null}
          {data.users && data.users.list.length ? (
            <div>
              ...
              {data.users.list.map(user => <User user={user} onChange={this.props.onChange} checked={this.getChecked(user)} key={'user' + user.accountId} />)}
            </div>
          ) : null}
        </div>
      );
