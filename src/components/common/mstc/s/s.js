...
const UploadFiles = require('src/components/UploadFiles').default;
  ...
  require(['voteUpdater'], function () {
    var MDUpdater = {
      ...
      handleUploadComplete(bool) {
        ...
        if (bool && (!value || value == (_l('知会工作是一种美德') + '...') || value == _l('上传附件')) && (MDUpdater.options.attachmentData.length || MDUpdater.options.kcAttachmentData.length)) {
          $textarea.val(MDUpdater.options.attachmentData.length ? MDUpdater.options.attachmentData[0].originalFileName : MDUpdater.options.kcAttachmentData[0].originalFileName);
          $textarea.focus();
        }
      },
      render() {
        ReactDom.render(<UploadFiles
            ...
            onTemporaryDataUpdate={ result => { MDUpdater.options.attachmentData = result } }
            onKcAttachmentDataUpdate={ result => { MDUpdater.options.kcAttachmentData = result } }
            onUploadComplete={ bool => { this.handleUploadComplete(bool) } }
      ...
      bindUploadEvent: function () {
        ...
        // MDUpdater.options.defaultPostType = 9;
        // this.render();
        /*
        require.async('uploadAttachment', function () {
          MDUpdater.options.uploadObj = $('#hidMDUpdaterUpload').uploadAttachment({
            pluploadID: '#mdUploadAttachment',
            showKcAttachmentEntry: true,
            dropPasteElement: 'MDUpdater_textarea_Updater',
            pastePasteElement: 'MDUpdater_textarea_Updater',
            arrowLeft: 2,
            defaultAttachmentData: MDUpdater.options.defaultAttachmentData,
            defaultKcAttachmentData: MDUpdater.options.defaultKcAttachmentData,
            callback: function (attachments, totalSize) {
              MDUpdater.options.attachmentData = attachments.attachmentData;
              MDUpdater.options.kcAttachmentData = attachments.kcAttachmentData;
              $('#MDUpdater_Div_JoinKnowledge').hide();
              //  待处理 赋值文件名
              // if (attachments.length > 0) {
              //   var $mdUpdaterTextareaUpdater = $('#MDUpdater_textarea_Updater');
              //   if (!$mdUpdaterTextareaUpdater.val() || $mdUpdaterTextareaUpdater.val() == langUploadFiles)
              //     $mdUpdaterTextareaUpdater.val(MDUpdater.options.attachmentData[0].originalFileName).removeClass('Gray_a').focus();

              //   // if (md.global.Project.haveKnowledge)
              //   //     $("#MDUpdater_Div_JoinKnowledge").show();
              // }
            },
            initAttactmentsComplete: function () {
            ...
            filesAdded: function () {
              ...
              if ((postType === '0' || postType === '9') && $('#MDUpdater_Attachment_updater').is(':hidden')) {
                $('#MDUpdater_hidden_UpdaterType').val(MDUpdater.options.postType.attachment);
                $('#MDUpdater_Attachment_updater').show();
              }
            },
            isUploadComplete: function (isUploadComplete) {
              // 所有文件上传进度
              if ( MDUpdater.options.uploadObj) {
                MDUpdater.options.uploadObj.isUploadComplete = isUploadComplete;
              }
            },
          });
        });
      ...
      resetUpdater: function (options, clearCallback) {
        ...
        // 投票
        $('#MDUpdater_Vote_updater').voteUpdater('reset');
      ...
      postUpdater: function (obj) {
        require(['mentioninput', 'selectGroup'], function () {
          var $mdUpdaterTextareaUpdater = $('#MDUpdater_textarea_Updater');
          ...
          $mdUpdaterTextareaUpdater.mentionsInput('val', function (data) {
            ...
            if (postType == MDUpdater.options.postType.link) {
              ...
              var voteData = $('#MDUpdater_Vote_updater').voteUpdater('getData');
              if (voteData.invalid) {
            ...
            postAjax.addPost(rData).then(function (result) {
              if (!result.success) {
                alert(_l('发布动态失败'), 2);
                return;
              }
      ...
      dialogChooseGroup: function (el, hidGroupID, projectId) {
        require(['src/components/dialogSelectGroup/dialogSelectGroup.js', 'selectGroup'], function () {
      ...
      showUpdaterDivForDocCenter: function (options) {
        ...
        require(
          ['dot', 'mdDialog', 'autoTextarea', 'selectGroup', 'mentioninput'],
          function (doT, mdDialog) {
            MDUpdater.dialog = mdDialog.index({
              dialogBoxID: 'easyDialogBoxMDUpdater',
              width: 640,
              container: {
                yesText: '',
                noText: '',
                content: doT.template(require('./s.html'))(),
              },
              readyFn: function () {
      ...
      bindEvent: function () {
        ...
        $mdUpdaterTextareaUpdater.focus(function (ev) {
          var msg = $mdUpdaterTextareaUpdater.val();
          if (msg == _l('知会工作是一种美德') + '...' ||
            msg == langUploadFiles ||
            msg == langShareLink ||
        ...
        $('div.MDUpdater a[targetDiv]').on('click', function () {
          ...
          if (targetDivID == '#MDUpdater_Attachment_updater') {
            if (MDUpdater.options.attachmentData.length == 0 && MDUpdater.options.kcAttachmentData.length == 0 && $('#MDUpdater_Attachment_updater').is(':visible')) {
              MDUpdater.resetUpdater();
              return;
          ...
          if (targetDivID == '#MDUpdater_Link_updater') {
            if (($('#MDUpdater_Link_updater').val().trim() == 'http://' || $('#MDUpdater_Link_updater').val().trim() == '') && $('#MDUpdater_Link_updater').is(':visible')) {
              MDUpdater.resetUpdater();
              return;
          ...
          if (targetDivID == '#MDUpdater_Vote_updater' && $('div.MDUpdater .voteOptions li').eq(0).find('input').val() == _l('请输入投票项') && $('div.MDUpdater .voteOptions li').eq(1).find('input').val() == _l('请输入投票项') && $('#MDUpdater_Vote_updater').is(':visible')) {
            MDUpdater.resetUpdater();
            return;
          ...
          if (targetDivID == '#MDUpdater_Attachment_updater') {
            ...
            $('#MDUpdater_hidden_UpdaterType').val(MDUpdater.options.postType.attachment);
            if ($('#MDUpdater_textarea_Updater') && ($('#MDUpdater_textarea_Updater').val().trim() == '' || $('#MDUpdater_textarea_Updater').val().trim() == _l('知会工作是一种美德') + '...')) {
            ...
            $('#MDUpdater_Vote_updater').voteUpdater('init');
          }
          else {
            $('#MDUpdater_hidden_UpdaterType').val(MDUpdater.options.postType.post);
          }
          ...
          // 关闭按钮
          if (targetDivID != '#MDUpdater_Video_updater')
            $('#mdUpdateCloseContainer').show();
        });
