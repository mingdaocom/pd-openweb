 ...
 * @param  {PluploadFile} file  文件对象
 * @param  {Object}  callback yesFn和noFn
 * @desc 根据文件内容显示的确认层，能预览图片或显示对应icon 返回打开的dialog对象
 * 弹层可修改文件的上传名称，直接修改file对象的name
 * @example
 * var fileConfirm = require('fileConfirm');
 * var fileDialog = fileConfirm(file, {
 *  yesFn: function (file) {}, //  返回file对象
 *  noFn: function (file) {},
 * });
 */
import './style.less';
import { formatFileSize, getClassNameByExt } from 'src/util';
var mainTpl = require('./main.htm');
