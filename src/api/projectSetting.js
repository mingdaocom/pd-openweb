...
1：系统消息；2：短信；3：邮件
  ...
  **/
   ...
   * 保存 组织成员字段显示/隐藏配置
   * @param {Object} args 请求参数
   * @param {array} args.psersonalSetList 个人 资料显示 字段（只接受 个人资料的 Type[1-50]）
   * @param {array} args.cardSetList 名片层 资料显示 字段（只接受 名片层的 Type[51-99]）
   * @param {} args.displayFieldForName
   * @param {string} args.projectId 组织Id
   * @param {Object} options 配置参数
   * @param {Boolean} options.silent 是否禁止错误弹层
   * @returns {Promise<Boolean, ErrorModel>}
