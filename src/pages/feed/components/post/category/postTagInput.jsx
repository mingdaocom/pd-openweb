    ...
    require(['textboxList'], () => {
        ...
        plugins: {
          autocomplete: {
            ...
            remote: {
              ...
              resultFilter: result =>
                (result.list || []).map(tag => [tag.id, htmlEncodeReg(tag.value), htmlEncodeReg(tag.value), htmlEncodeReg(tag.value)]),
      ...
      for (let j = 0; j < presetValue.length; j++) {
        if ($.trim(presetValue[j]).length) {
          tblCategory.add(null, presetValue[j].split('|')[0], presetValue[j].split('|')[1], null, true, { deleteButton: true });
        }
      }
      if (andLink) {
        tblCategory
...
module.exports = connect(state => ({}))(PostTagInput);
