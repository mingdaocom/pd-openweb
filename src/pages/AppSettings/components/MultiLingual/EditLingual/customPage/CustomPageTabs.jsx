...
export default function CustomPageView(props) {
  ...
  useEffect(() => {
    ...
    customApi
      ...
      .then(data => {
        ...
        setLoading(false);
        setList(components.filter(c => c.type === 9));
      });
  }, [selectNode.key]);
  ...
  if (!list.length) {
    return (
      <div className="flexRow alignItemsCenter justifyContentCenter h100 textTertiary Font14">{_l('没有标签页')}</div>
    );
  }
  ...
  const renderContent = item => {
    ...
    const handleSave = info => {
      onEditAppLang({
        ...
        type: LANG_DATA_TYPE.customePageTabs,
        data: {
          ...translateInfo,
          ...info,
        },
    ...
    return (
      <div className={cx('flexColumn mBottom30', `navItem-${item.id}`)} key={item.id}>
        ...
        <div className="flexRow nodeItem">
          <div className="Font13 mRight20 label">{_l('标签页名称')}</div>
          <div className="flex">
            {item.componentConfig.tabs.map(item => (
