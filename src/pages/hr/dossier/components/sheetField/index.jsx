...
class SheetField extends Component {
  ...
  render() {
    ...
    if (this.props.value) {
      ...
      if (
        sourceControlType === 26 ||
        sourceControlType === 27 ||
        sourceControlType === 14 ||
        sourceControlType === 21
      ) {
      ...
      switch (this.props.sourceControlType) {
        case 2:
          return (
            <div className="mui-textview Gray_75 WordBreak">
              ...
              {value.join
                ? value
                    .map(
                      item =>
                        (item.indexOf('.') > -1
                          ? item.toString().replace(/(\d)(?=(\d{3})+\.)/g, '$1,')
                          : item.toString().replace(/(\d)(?=(\d{3})+$)/g, '$1,')) +
        ...
        case 14: {
          ...
          return <UploadFiles isUpload={false} attachmentData={attachmentData} />;
        }
        case 15:
          return (
            <div className="mui-textview Gray_75 WordBreak">
          ...
          return (
            <div className="mui-textview Gray_75 WordBreak">
              {value.map
                ? value
                    .map(valueItem => {
                      return valueItem ? JSON.parse(valueItem)[0].departmentName : '';
                    })
                    .join('、')
                : value}
            </div>
          ...
          return (
            <div className="mui-textview Gray_75 WordBreak">
              {value.join
                ? value
                    .map(item => {
                      return this.props.enumDefault === 1 ? item + '星' : item + '级';
                    })
                    .join('、')
                : value}
            </div>
        ...
        default: {
          ...
          return <div className="mui-textview Gray_75 WordBreak">{value.join ? value.join('、') : value}</div>;
        }
      }
    }
    return null;
