...
export default function FilterControl(props) {
  ...
  useEffect(() => {
    const request = filterObjectControls
      .filter(item => {
      ...
      Promise.all(request).then(data => {
        ...
        data = data.map(sheet => {
          ...
          _.set(
            sheet,
            'template.controls',
            replaceControlsTranslateInfo(sheet.appId, sheet.worksheetId, controls).map(redefineComplexControl),
          );
  ...
  const renderSelect = (item, index) => {
    ...
    const firstControlData =
      _.find(_.get(firstSheet, 'template.controls'), { controlId: _.get(lastControl, 'controlId') }) || {};
  ...
  return (
    <Fragment>
      ...
      <div>
        {filterObjectControls.map((item, index) => renderSelect(item, index))}
