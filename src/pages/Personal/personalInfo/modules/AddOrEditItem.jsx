...
export default class AddOrEditItem extends React.Component {
  ...
  getRangeTime(type) {
    const day1 = new Date(type === 'max' ? new Date().getTime() - 24*60*60*1000 : new Date(this.state.baseInfo.startDate).getTime() + 24*60*60*1000)
  ...
  saveBaseInfo() {
    const {errorList, baseInfo = {}} = this.state
    let isError = false
    ...
    return (
      <div className="baseInfoEditContent Gray">
        ...
        <input
          ...
          onBlur={() => this.updateError('name', !baseInfo.name)}
          onFocus={() => this.updateError('name', false)}
        />
        <div className="Red errorBox">
          <span className={cx({ Hidden: !errorList.name })}>{type === 1 ? _l('请输入组织名称') : _l('请输入学校名称')}</span>
        </div>
        {/**专业学历和职位 */}
        <div className="Bold">
        ...
        <input
          ...
          onBlur={() => this.updateError('title', !baseInfo.title)}
          onFocus={() => this.updateError('title', false)}
        />
        <div className="Red errorBox">
          <span className={cx({ Hidden: !errorList.title })}>{this.renderTitleErrorMsg()}</span>
        </div>
        {/**描述或核心课程 */}
        ...
        <div className="flexRow mTop6">
          ...
          <div>
            ...
            <div className="Red errorBox">
              <span className={cx({ Hidden: (!errorList.endDate&&!baseInfo.isSoFar) || baseInfo.isSoFar })}>{_l('请选择结束年月')}</span>
            </div>
          </div>
        </div>
        <Checkbox checked={baseInfo.isSoFar} onClick={() => {
