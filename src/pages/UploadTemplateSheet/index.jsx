        ...
        <div className="con">
          ...
          <div className="mTop20">
            ...
            <p className="textSecondary">
              <span>
                {_l(
                  '5. 可通过代码 #{[qrcode]字段名$[20*20_H]$} 或 #{[barcode]字段名$[40*10]$}，获取任意字段的二维码或条形码。其中，[20*20]、[40*10] 表示二维码或条形码的宽高尺寸，单位为 mm。二维码采用 QR Code 编码，最多支持 150 个字符，支持汉字；条形码采用 Code 128 编码，最多支持 30 个字符，支持数字、字母及常用符号。',
                )}
              </span>
              <ol className="">
                <li className="textSecondary pLeft12">
                  {_l(
                    '（1）二维码尺寸为固定尺寸。[20*20] 表示二维码宽度和高度均为 20 mm。二维码需保持正方形显示，不支持 auto 自适应尺寸。',
                  )}
                </li>
                <li className="textSecondary pLeft12">
                  <div>
                    {_l(
                      '（2）二维码支持配置容错率，在尺寸后添加容错率代码，例如：#{[qrcode]字段名$[20*20_H]$}。支持以下两种写法：',
                    )}
                  </div>
                  <div>{_l('字母代码：L、M、Q、H')}</div>
                  <div>{_l('数值代码：7、15、25、30，也兼容 7%、15%、25%、30%')}</div>
                  <div>
                    {_l(
                      '对应关系为：L = 7%、M = 15%、Q = 25%、H = 30%。容错率越高，二维码被遮挡或污损后仍可识别的能力越强，但可容纳的有效内容会相应减少。',
                    )}
                  </div>
                  <div>
                    {_l(
                      '例如：#{[qrcode]字段名$[20*20_Q]$}、#{[qrcode]字段名$[20*20_25]$} 与 #{[qrcode]字段名$[20*20_25%]$}，均表示生成尺寸为 20 × 20 mm、容错率为 25% 的二维码。',
                    )}
                  </div>
                </li>
                <li className="textSecondary pLeft12">
                  {_l(
                    '（3）条形码支持 4 种尺寸设置方式：[40*auto] 表示宽度固定为 40 mm，高度按比例自适应；[auto*10] 表示高度固定为 10 mm，宽度按比例自适应；[40*10_auto] 表示宽高均为 40 × 10 mm，并根据内容自适应；[40*10] 表示宽高固定为 40 × 10 mm。',
                  )}
                </li>
                <li className="textSecondary pLeft12">
                  {_l(
                    '（4）条形码默认显示下方文字。如需隐藏条形码下方的文字，可增加 _NT 控制参数，例如：#{[barcode]字段名$[40*10_NT]$}。未添加 _NT 时，仍按原有方式显示文字。',
                  )}
                </li>
                <li className="textSecondary pLeft12">
                  {_l(
                    '（5）示例：#{[qrcode]字段名$[20*20_H]$} 表示生成尺寸为 20 × 20 mm、容错率为 30% 的二维码；#{[barcode]字段名$[40*auto]$} 表示条形码宽度固定为 40 mm、高度按比例自适应；#{[barcode]字段名$[auto*10]$} 表示条形码高度固定为 10 mm、宽度按比例自适应；#{[barcode]字段名$[40*10_NT]$} 表示生成固定尺寸为 40 × 10 mm 的条形码，并隐藏下方文字。',
                  )}
                </li>
              </ol>
            </p>
