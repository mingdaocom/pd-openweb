...
var flowNode = {
   ...
   * @param {SaveEditNode} {accounts:候选用户  3填写,4审批(array),allowTransfer:允许转交他人填写  3填写(boolean),flowNodeType:节点ID(integer),formProperties:表单字段属性  3填写(array),name:节点名称(string),nodeId:节点ID(string),processId:流程ID(string),schedule:限时配置(ref),selectNodeId:选中的节点ID(string),submitBtnName:提交按钮名称(string),}*填写
   * @param {SaveApproveNode} {accounts:审批的人员(array),allowForward:允许审批人转审  4审批(boolean),allowSign:允许审批人加签  4审批(boolean),auth:审批需要的动作(ref),batch:允许批量审批(boolean),callBackType:0: 重新执行  1:直接返回审批节点(integer),condition:会签通过的表达式，按比例通过(string),countersign:是否是会签组(boolean),countersignType:会签类型  1 全员通过 2单个通过 3 或签(integer),flowNodeType:节点ID(integer),formProperties:表单字段属性  3填写(array),ignoreRequired:忽略必填项(boolean),isCallBack:是否勾选回退按钮(boolean),multipleLevel:层级数 -1为最高级(integer),multipleLevelType:是否是层级及类型 0：不是层级 1：部门负责人  2：主部门负责人(integer),name:节点名称(string),nodeId:节点ID(string),operationTypeList:允许操作的类型列表 跟详情的操作对应(array),overruleBtnName:拒绝按钮名称(string),passBtnName:通过按钮名称(string),processId:流程ID(string),schedule:限时配置(ref),selectNodeId:选中的节点ID(string),signatureType:1:审批通过后，必须签名(integer),updateAllowTask:允许审批人修改填写内容  4审批(boolean),}*审批
