...
var flowNode = {
   ...
   * @param {SaveMultipleNode} {actionId:获取方式  400: 从工作表获取多条记录 , 401: 从一条记录获得多条关联记录, 402 :从新增节点获取多条记录，403:从数组获取多条数据(string),appId:选择其他对象时，选中的对象ID(string),execute:是否立即执行(boolean),fields:配置的字段信息(array),flowNodeType:节点ID(integer),name:节点名称(string),nodeId:节点ID(string),numberFieldValue:最多获取条数(ref),operateCondition:筛选条件(array),processId:流程ID(string),selectNodeId:选中的节点ID(string),sorts:查排序规则(array),}*批量
   ...
   * @param {SaveTemplateMessageNode} {accounts:通知发送的人员(array),appId:模板id(string),fields:配置的字段信息(array),flowNodeType:节点ID(integer),name:节点名称(string),nodeId:节点ID(string),processId:流程ID(string),templateNode:跳转url  和小程序地址(ref),}*微信模板消息
