...
var flowNode = {
   ...
   * @param {SavePushNode} {actionId:默认执行的动作  5： 新增草稿(string),appId:工作表id(string),buttons:按钮配置(array),content:提示内容  或者 链接url(string),duration:自动消失：0代表永不关闭需要手动关闭；默认5s(integer),fields:配置的字段信息(array),flowNodeType:节点类型(integer),name:节点名称(string),nodeId:节点ID(string),openMode:打开方式：1:当前页面 2:新页面,3:弹层 4:模态窗口(integer),processId:流程ID(string),promptType:1:成功，2：失败，3：警告，4：通知(integer),pushType:推送内容类型 1:弹出提示,2:创建记录,3:打开记录详情,4:打开视图,5:打开自定义页面,6:打开链接(integer),selectNodeId:选中的节点ID(string),title:标题(string),viewId:视图id(string),}*推送
   ...
   * @param {Object} args 请求参数
   * @param {string} [args.access_token] 令牌
   * @param {WebHooksRequest} {authId:授权账号id(string),body:body(string),contentType:contentType  1: key-value 2:json(integer),formControls:formControls(array),headers:请求头(array),json:importJosn(string),method:请求方法 1:get  2:post(integer),nodeId:节点ID(string),params:params(array),processId:流程Id(string),settings:setting(ref),url:url(string),}*webHooksRequest
   * @param {Object} options 配置参数
   * @param {Boolean} options.silent 是否禁止错误弹层
   */
