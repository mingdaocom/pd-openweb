...
var flowNode = {
   ...
   * @param {SaveCodeNode} {actionId:动作id 102:Javascript，103:Python(string),code:代码块(string),flowNodeType:节点ID(integer),inputDatas:input输入 map(array),maxRetries:重试次数(integer),name:节点名称(string),nodeId:节点ID(string),processId:流程ID(string),selectNodeId:选中的节点ID(string),testMap:测试值(object),version:运行版本(string),}*代码块
   * @param {SaveLinkNode} {addNotAllowView:新增不允许查看,true代表不允许查看，老的默认是可以查看(boolean),flowNodeType:节点ID(integer),formProperties:表单字段属性  3填写(array),linkName:提交按钮名称(string),linkType:链接类型 1:分享链接 2:填写链接(integer),modifyTime:修改的失效时间(integer),name:节点名称(string),nodeId:节点ID(string),password:密码(string),processId:流程ID(string),selectNodeId:选中的节点ID(string),submitButtonName:提交按钮名称(string),submitType:提交后是否运行查看：1:仅查看，2：允许修改(integer),time:失效时间配置(ref),}*获取记录链接
   ...
   * @param {SavePrintNode} {appId:打印模板id(string),fileName:文件名(string),flowNodeType:节点ID(integer),name:节点名称(string),nodeId:节点ID(string),pdf:是否勾选pdf(boolean),processId:流程ID(string),selectNodeId:选中的节点ID(string),}*获取记录打印文件
