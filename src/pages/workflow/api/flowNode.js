...
var flowNode = {
   ...
   * @param {增加节点} {actionId:默认执行的动作ID 1:新增  2： 修改(string),all:复制分支下所有的节点(boolean),appId:已有审批流id/代码块模版id(string),appType:应用类型(integer),gatewayType:分支类型(integer),isMove:移到分支一侧： true，不移动： false(boolean),moveType:移到分支一侧 0:不移动 1：左侧 2：右侧 3撤回分支(integer),name:节点名称(string),nodeIds:复制的节点ids(array),processId:流程ID(string),prveId:前一个节点ID(string),resultFlow:是审批结果分支类型(默认false)(boolean),typeId:流程节点类型ID(integer),}*addFlowNode
  ...
  createCodeTemplate: function(args, options) {
    ...
    base.ajaxOptions.type = 'POST';
    return $.api(controllerName, 'flowNodecreateCodeTemplate', JSON.stringify(args), $.extend(base, options));
  },
  /**
   * 创建短信模版
   * @param {Object} args 请求参数
  ...
  getCodeTemplateList: function(args, options) {
    ...
    base.ajaxOptions.type = 'GET';
    return $.api(controllerName, 'flowNodegetCodeTemplateList', args, $.extend(base, options));
  },
  /**
   * 筛选节点赋值的下拉列表
   * @param {Object} args 请求参数
  ...
  jsonToControls: function(args, options) {
    ...
    base.ajaxOptions.type = 'POST';
    return $.api(controllerName, 'flowNodejsonToControls', JSON.stringify(args), $.extend(base, options));
  },
  /**
   * 节点说明
   * @param {Object} args 请求参数
   ...
   * 保存节点的配置信息
   * @param {Object} args 请求参数
   * @param {SaveStartNode} {appId:引用对象ID(string),appType:应用类型(integer),assignFieldId:指定字段(string),assignFieldIds:指定字段(array),config:配置(ref),controls:webhook触发开始的字段配置(array),endTime:结束时间(string),executeEndTime:执行结束时间(string),executeTime:执行时间(string),executeTimeType:执行时间的类型 0:按以上日期 1：之前 2：之后(integer),fields:配置的字段信息(array),flowNodeMap:其他节点功能(object),flowNodeType:节点ID(integer),frequency:重复频率 1:天 2:周 3:月 4:年(integer),hooksBody:是否需要body全文(boolean),interval:重复间隔 间隔单位frequency(integer),isUpdate:是否修改(boolean),name:节点名称(string),nodeId:节点ID(string),number:设置的时间(integer),operateCondition:触发条件(array),params:key-value的参数(array),processConfig:审批配置(ref),processId:流程ID(string),repeatType:循环类型 1:每天 2:工作日 3:每周 4:每月 5:每年 6:自定义(integer),returnJson:返回值自定义的json(string),returnMap:null(object),returns:自定义的返回值 key-value(array),selectNodeId:选中的节点ID(string),time:时间(string),triggerId:触发方式(string),unit:单位：1 : 分，2 ：小时 3:天(integer),weekDays:周几 0:周天 1:周1...（需按顺序[0, 1, 2, 3, 4, 5, 6]）(array),}*开始
   ...
   * @param {SaveEmailNode} {accounts:通知发送的人员(array),actionId:默认执行的动作   1： 新增 2：更新,20:获取关联他表 201:发送邮件(string),appType:选择其他对象时，应用类型 0:默认第3方,1:工作表,2:任务,3:邮件(integer),batch:审批节点邮件，允许批量审批(boolean),ccAccounts:通知抄送人(array),fields:配置的字段信息(array),flowNodeType:节点ID(integer),name:节点名称(string),nodeId:节点ID(string),processId:流程ID(string),selectNodeId:选中的节点ID(string),}*邮件
   * @param {SaveTimerNode} {actionId:默认执行的动作  300： 延时到指定日期   301：延时一段时间(string),executeTimeType:执行时间的类型 0:按以上日期 1：之前 2：之后(integer),fieldControlId:日期字段控件ID(string),fieldNodeId:输入日期字段节点ID(string),fieldValue:输入日期的值(string),flowNodeType:节点ID(integer),hour:小时(integer),hourFieldValue:动态小时的值(ref),minute:分钟(integer),minuteFieldValue:动态分钟的值(ref),name:节点名称(string),nodeId:节点ID(string),number:设置的时间(integer),numberFieldValue:动态天的值(ref),processId:流程ID(string),secondFieldValue:动态秒的值(ref),selectNodeId:选中的节点ID(string),time:时间(string),unit:单位：1 : 分，2 ：小时 3:天(integer),}*延时
   * @param {SaveMultipleNode} {actionId:获取方式  400: 从工作表获取多条记录 , 401: 从一条记录获得多条关联记录, 402 :从新增节点获取多条记录，403:从数组获取多条数据(string),appId:选择其他对象时，选中的对象ID(string),execute:是否立即执行(boolean),fields:配置的字段信息(array),filters:新的筛选组(array),flowNodeType:节点ID(integer),name:节点名称(string),nodeId:节点ID(string),numberFieldValue:最多获取条数(ref),operateCondition:筛选条件(array),processId:流程ID(string),relation:是否需要汇报关系(boolean),selectNodeId:选中的节点ID(string),sorts:查排序规则(array),}*批量
   * @param {SaveCodeNode} {actionId:动作id 102:Javascript，103:Python(string),code:代码块(string),flowNodeType:节点ID(integer),inputDatas:input输入 map(array),name:节点名称(string),nodeId:节点ID(string),processId:流程ID(string),selectNodeId:选中的节点ID(string),testMap:测试值(object),}*代码块
   * @param {SaveLinkNode} {flowNodeType:节点ID(integer),formProperties:表单字段属性  3填写(array),linkName:提交按钮名称(string),linkType:链接类型 1:分享链接 2:填写链接(integer),name:节点名称(string),nodeId:节点ID(string),password:密码(string),processId:流程ID(string),selectNodeId:选中的节点ID(string),submitButtonName:提交按钮名称(string),time:失效时间配置(ref),}*获取记录链接
   ...
   * @param {SavePushNode} {appId:工作表id(string),buttons:按钮配置(array),content:提示内容  或者 链接url(string),duration:自动消失：0代表永不关闭需要手动关闭；默认5s(integer),flowNodeType:节点ID(integer),name:节点名称(string),nodeId:节点ID(string),openMode:打开方式：1:当前页面 2:新页面,3:弹层 4:模态窗口(integer),processId:流程ID(string),promptType:1:成功，2：失败，3：警告，4：通知(integer),pushType:推送内容类型 1:弹出提示,2:创建记录,3:打开记录详情,4:打开视图,5:打开自定义页面,6:打开链接(integer),selectNodeId:选中的节点ID(string),title:标题(string),viewId:视图id(string),}*推送
  ...
  updateCodeTemplate: function(args, options) {
    ...
    base.ajaxOptions.type = 'POST';
    return $.api(controllerName, 'flowNodeupdateCodeTemplate', JSON.stringify(args), $.extend(base, options));
  },
  /**
   * 更新节点名称
   * @param {Object} args 请求参数
