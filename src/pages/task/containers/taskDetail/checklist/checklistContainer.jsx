...
let ChecklistContainer = class ChecklistContainer extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps !== this.props) {
      ...
      this.setState({
        source,
        showAddItemChecklistId:
          this.props.addChecklist && source.length > this.state.source.length
            ? source[source.length - 1].checkListId
            : '',
      });
      if (this.props.addChecklist && source.length > this.state.source.length) {
        this.props.closeAddChecklist();
      }
    }
  }
  ...
  checklistItemHover(index, topIndex) {
    ...
    if (source[topIndex].items.length < 100) {
      ...
      source[topIndex].items.splice(index, 0, {
        type: 'blank',
        status: this.dragItem.status,
      });
      this.setState({
