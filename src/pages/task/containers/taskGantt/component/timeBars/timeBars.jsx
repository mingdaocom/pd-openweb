...
const ganttSource = {
  beginDrag(props, monitor, component) {
    ...
    const dargDom = $(component.getNode()).find('.timeBar')[0];
  ...
  isDragging(props, monitor) {
    ...
    if (preview && clientOffset) {
      ...
      preview.style.top = clientOffset.y - config.offset.y + 'px'; // 当前scrollLeft值
      ...
      config.diffHours = diffHours; // 无调整用原始时间显示
      ...
      if (diffHours === 0) {
        ...
        config.newEndTime = newTimes.end; // 填充开始时间
      ...
      config.setInterval = setInterval(() => {
        ...
        const left = clientOffset.x - config.offset.x; // 竖着滚动
...
let TimeBars = class TimeBars extends Component {
  ...
  dragMouseMove(direction) {
    ...
    $(document).on({
      'mousemove.ganttDragSingle': function (evt) {
        evt.stopPropagation(); // 当前scrollLeft值
        ...
        clearInterval(config.setInterval); // 拖拽左侧
        ...
        if (direction === config.DRAG_DIRECTION.LEFT) {
          ...
          config.newStartTime = start; // 更新开始时间

          that.props.updateStartTimeAndEndTime(config.singleDragTaskId, that.props.index, start, direction); // 处理滚动条
        ...
        if (direction === config.DRAG_DIRECTION.RIGHT) {
          ...
          config.newEndTime = end; // 更新结束时间

          that.props.updateStartTimeAndEndTime(config.singleDragTaskId, that.props.index, end, direction); // 处理滚动条

          that.dragTriggerScroll(evt);
        }
      },
      'mouseup.ganttDragSingle': function (evt) {
        evt.stopPropagation(); // 修改开始时间

        if (direction === config.DRAG_DIRECTION.LEFT && config.newStartTime) {
          // 更新
          that.props.updateTaskStartTimeAndDeadline(config.singleDragTaskId, true, config.newStartTime, ''); // 数据先恢复到初始状态
        ...
        if (direction === config.DRAG_DIRECTION.RIGHT && config.newEndTime) {
          // 更新
          that.props.updateTaskStartTimeAndDeadline(config.singleDragTaskId, false, '', config.newEndTime); // 数据先恢复到初始状态
  ...
  getBGColor() {
    const { data } = this.props; // 有效时长是0时无颜色
  ...
  timeBarArrowStyle() {
    const { data } = this.props; // 只有开始时间 箭头画右边
  ...
  getOverdueMessage() {
    const { data } = this.props; // 拖的时候不呈现超期文案
  ...
  onMouseOver(evt) {
    let offsetX = evt.clientX - $(evt.currentTarget).offset().left - $(evt.currentTarget).outerWidth() / 2; // 右边不够放
        ...
        >
              ...
              >
                <div>
                  {timeBarStyle.width < 8 ? undefined : (
                    <i
                      className={cx('timeBarDragLeft', {
                        dragBigRadius: timeBarStyle.width >= 72,
                      })}
          ...
          {data.deadline ? (
            <span
              ...
              style={{
                width: utils.getOneHourWidth(viewType) * this.getOverdueHour(),
              }}
            >
