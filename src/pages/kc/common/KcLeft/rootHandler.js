...
export function addNewRoot(args, cb) {
  ...
  require(['src/components/kc/createRoot/createRoot'], (createRoot) => {
    let params = {
      members: [
        {
...
export function editRoot(rootId, successCb, progressCb) {
  require(['src/components/kc/createRoot/createRoot'], (editRoot) => {
    editRoot({
      isEdit: true,
      id: rootId,
    })
...
export function removeRoot(item, isCreator, isPermanent, cb) {
  ...
  DeleteConfirm({
    ...
    description: isCreator ? _l('共享文件夹将彻底删除，同时删除共享文件夹下所有文件夹和文件，且无法恢复。请确认您和文件夹中的成员都不再需要共享文件夹中的数据再执行此操作') : '',
