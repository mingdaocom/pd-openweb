...
const fetchJobUsers = (projectId, jobId, pageIndex) => {
  const params = {
    pageIndex,
    jobId,
    projectId,
...
const fetchJobList = (projectId, jobId, pageIndex = 1) => {
  const params = {
    projectId,
    pageIndex,
    pageSize: PAGE_SIZE,
...
//获取职位列表
export const loadJobList = (projectId, jobId, pageIndex) => dispatch => {
  return dispatch(fetchJobList(projectId, jobId, pageIndex));
};
