...
export default class AppManagement extends Component {
  ...
  getAppList() {
    ...
    this.postList.then(({ apps, maxCount, total, count }) => {
      ...
      this.props.updateListTotalNum(total);
    });
  }
  ...
  renderListItem(item) {
    ...
    return (
      <div className="flexRow manageList overflowHidden" key={item.appId}>
            ...
            >
              ...
              {item.sourceType === 60 && (
                <Tooltip title={_l('通过市场安装')}>
                  <Icon icon="merchant" className="textDisabled mLeft20 Font16" />
                </Tooltip>
              )}
              {item.isLock && (
                <Tooltip title={_l('应用已锁定')}>
                  <Icon icon="lock" className="textDisabled mLeft20 Font16" />
                </Tooltip>
              )}
            </div>
          </div>
        ...
        <div className="columnWidth textTertiary">{moment(item.ctime).format('YYYY-MM-DD')}</div>
    ...
    ajaxRequest
      ...
      .then(result => {
        if (result) {
          ...
          this.setState({ list, transferLoading: false });
        } else {
          this.setState({ transferLoading: false });
        }
      })
  ...
  render() {
    ...
    return (
      <Fragment>
        <div className={`appManagementContent flexColumn appManagementList ${className}`}>
          {maxCount > 0 && (_.isEmpty(version) || version.versionIdV2 === '1') && (
            <div className="appManagementCount flexRow">
              <span className="textTertiary mRight5">
