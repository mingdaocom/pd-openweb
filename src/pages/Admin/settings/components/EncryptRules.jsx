...
import { Icon, Input, Switch, Menu, MenuItem, Tooltip, ScrollView, LoadDiv } from 'ming-ui';
...
export default class EncryptRules extends Component {
  ...
  getDataList = () => {
    ...
    const { searchValues, pageIndex, loading, pageSize, isMore } = this.state;
    // 加载更多
    ...
    this.promise
      .then(res => {
        this.setState({
          dataSource: pageIndex === 1 ? res.encryptRules : this.state.dataSource.concat(res.encryptRules),
          loading: false,
          isMore: res.encryptRules.length >= pageSize,
  ...
  render() {
    ...
    const { searchValues, dataSource = [], showAddEditDialog, loading, pageIndex } = this.state;
    const { type, state, name } = searchValues;
    const featureType = getFeatureStatus(projectId, 29);
      ...
      <div className="orgManagementWrap">
        <div className="orgManagementHeader">
          ...
          <div
            ...
            onClick={() => {
              if (featureType === '2') {
                buriedUpgradeVersionDialog(projectId, 29);
                return;
              }
        ...
        <div className="orgManagementContent flex flexColumn pTop16">
          ...
          <div className="flex flexColumn mTop16 mBottom16 listContent">
            <ScrollView className="flex" onScrollEnd={this.onScrollEnd}>
              {dataSource.map(item => (
                <div className="flexRow listItem">
                  ...
                  <div className="w150">{_.get(_.find(encryptList, it => it.value === item.type) || {}, 'label')}</div>
