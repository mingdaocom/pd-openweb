...
import { emptyJobUserSet, loadJobUsers, loadJobList, updateSelectJobUser, updateAllSelectJobUser } from '../../actions/jobs';
...
class JopList extends React.Component {
    ...
    return <React.Fragment>
      ...
      <div className='actionBox Right'>
        <span onClick={(e) => {
          ...
          <Icon className="Font16 listName mRight15" icon="delete_out" />{_l('移出')}
        </span>
        <span onClick={(e) => {
          this.handleExportUser()
        }} className='selectedAccountAction Hand mLeft40 Font15'>
          <Icon className="Font16 listName mRight15" icon="Export_user" />{_l('导出选中用户')}
        </span>
      </div>
    </React.Fragment>
  }
  ...
  renderHeader = () => {
    const { jobId, projectId, dispatch, jobName, userIds = [], allCount, pageIndex, pageSize, isSelectAll } = this.props;
    return <React.Fragment>
      {(userIds.length > 0 || isSelectAll) ?
        <div className="Font15 departmentTitle">
          {this.renderUserTableWithNum()}
        </div> : ''}
      {(userIds.length <= 0 && !isSelectAll) ? (
        <div className="Font15 departmentTitle">
          <span className={classNames('overflow_ellipsis WordBreak')} style={{
            maxWidth: 500,
          ...
          {allCount > 0 ? <Pager pageFn={(index) => {
            dispatch(loadJobUsers(projectId, jobId, index))
          }}
            allCount={allCount}
            pageIndex={pageIndex}
            pageSize={pageSize}
      ...
      <thead>
        <tr>
          ...
          <th width="50px" className="pRight15">
            <span className="Hand mLeft24 ThemeColor3 addMenberBtns" onClick={(evt) => {
              this.addUser(evt)
            }}>
  ...
  renderUsers = () => {
    ...
    return user.map(item => {
      ...
      return <tr key={item.accountId} className={classNames("userItem", { isChecked: isCheck })}>
              ...
              <tr>
                ...
                <td className="TxtMiddle">
                  <div className="name mLeft10">
                    <a href={'/user_' + item.accountId} className="Gray" title={item.fullname}>{item.fullname}</a>
                  </div>
                </td>
              </tr>
            </tbody>
  ...
  renderNullList = () => {
    return <div className="TxtCenter noData">
      <div>
        <div className="nullState"><Icon className="" icon="Empty_Noposition" /></div>
        <p className="Gray_c mTop20">{_l('可以根据成员属性去创建职位，如，技术、生产、销售，设置后应用和工作流可以直接选择职位')}</p>
      </div>
    </div>
  }
  ...
  renderNullState = () => {
    return <div className="TxtCenter noData">
      <div>
        <div className="nullState"><Icon className="" icon="Empty_data" /></div>
  ...
  renderCon = () => {
    ...
    return isUserLoading ? <LoadDiv /> : <div>
      {<table className="fixedTable usersTable" cellSpacing="0">
        {this.renderThead()}
        <tbody>
          {user.length <= 0 ? this.renderNullState() :
            this.renderUsers()}
        </tbody>
      </table>}
    </div>;
  ...
  render() {
    ...
    return <div className='jobList'>
      {jobList.length > 0 ? <React.Fragment>
