  ...
  renderWX = () => {
    return (
      <React.Fragment>
        ...
        <h3 className="Font18 textPrimary mTop40">{_l('5. 配置完成后，即可在客户端使用此应用')}</h3>
  ...
  renderScanContent = () => {
    ...
    return (
      <div className="scanWorkwx" style={{ height: 2524 }}>
        <h3 className="Font18 textPrimary mTop40">{_l('钉钉扫码登录（可选）')}</h3>
        ...
        <img src={scan1} />
        <p className="Font14 textSecondary mTop24 LineHeight22">{_l('2.填写重定向 URL（授权回调域名）')}</p>
        <div className="inputList mTop20">
      ...
      return (
        <div className="courseBox card TxtCenter">
          ...
          <div className="TxtCenter mTop30">
            <h2 className="Font20 textPrimary">
              {this.state.isWX
                ? _l('抱歉，此功能只开放给基于企业微信开通的组织')
                : _l('抱歉，此功能只开放给基于钉钉开通的组织')}
            </h2>
            <p className="mTop16 mBottom30 Font14 textSecondary LineHeight22">
    ...
    if (this.state.isWX && !window.platformENV.isOverseas && !window.platformENV.isLocal) {
      return (
        <div className="wechartWork card TxtCenter">
          ...
          <div className="TxtCenter mTop30">
            <h2 className="Font20 textPrimary">{_l('将此应用添加到企业微信工作台')}</h2>
            <p className="mTop30 Font14 textSecondary LineHeight22">
              {_l('因企业微信规则调整，新的对接方案正在开发，支持线下完成对接。')}
            </p>
            <p className="Font14 mBottom30 textSecondary LineHeight22">
