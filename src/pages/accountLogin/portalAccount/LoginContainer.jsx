...
import { clickErrInput, getEmailOrTel, getDialCode, isTel } from 'src/pages/accountLogin/util.js';
      ...
      .then(res => {
        ...
        switch (accountResult) {
          ...
          case -3:
            ...
            updateWarn([{ tipDom: '.passwordIcon', warnningText: passwordRegexTip || _l('8-20位，需包含字母和数字') }]);
            setState({
              sending: false,
            });
            break;
          case -2:
            //-2代表密码错误校验失败；
            updateWarn([{ tipDom: '.passwordIcon', warnningText: _l('密码错误校验失败') }]);
            setState({
              sending: false,
            });
            break;
      ...
      <React.Fragment>
        ...
        <React.Fragment>
          <div
            ...
            onClick={async () => {
              let isV = await isValid(false, [way, 'code']);
              if (isV) {
