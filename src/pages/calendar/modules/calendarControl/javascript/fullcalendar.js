...
import moment from 'moment';
    ...
    function Calendar(element, instanceOptions) {
        ...
        function initialRender() {
            ...
            var $viewBox = $('<div class="fc-view-container-box"></div>');
            $viewBox.prependTo(element);
            content = $("<div class='fc-view-container'/>").prependTo($viewBox);
        ...
        function destroy() {
            ...
            header.destroy();
            content.remove();
            element.removeClass('fcOld fc-ltr fc-rtl fc-unthemed ui-widget');
            ...
            if (currentView) {
				...
				!date.isWithin(currentView.intervalStart, currentView.intervalEnd) // implicit date window change
                    ...
                    if (elementVisible()) {
                        ...
                        if (defaults.firstLoading) {
                            getAndRenderEvents();
                            defaults.firstLoading = false;
                        }
                        //getAndRenderEvents();
                    }
                }
    ...
    function EventManager(options) { // assumed to be a calendar
        ...
        function _fetchEventSource(source, callback) {
            ...
            if (events) {
                ...
                if (url) {
                    ...
                    if (startParam) {
                        data[startParam] = moment(rangeStart.format()).format();
                    }
                    if (endParam) {
                        data[endParam] = moment(rangeEnd.format()).format();
                    }
                    ...
                    //新加载数据方法
                    source.url(data).then(function(events){
                        ...
                        events.forEach((item) => {
                            if (!item.allDay && moment(item.startTime).format('YYYY-MM-DD') !== moment(item.endTime).format('YYYY-MM-DD')) {
                                item.allDay = true;
                            }

                            if (item.allDay && !item.isAllDay && moment(item.startTime).format('YYYY-MM-DD') === moment(item.endTime).format('YYYY-MM-DD')) {
                                item.allDay = false;
                            }
                        });
                        callback(events);
        ...
        function buildEvent(data, source) { // source may be undefined!
            ...
            var allDay;
            var allDayDefault;
            var isAddOneDay = moment(data.start).format('YYYY-MM-DD') !== moment(data.end).format('YYYY-MM-DD') && data.start.substr(0, 10) === data.end.substr(0, 10);
            var isReduceOneDay = moment(data.start).format('YYYY-MM-DD') === moment(data.end).add('d', -1).format('YYYY-MM-DD') && data.start.substr(0, 10) !== data.end.substr(0, 10)
        ...
        function mutateEvent(event, newStart, newEnd) {
            ...
            //修改：04/27/2016 Cloud 当为特殊子日程时，不修改其他日程的位置
            undoFunc = mutateEvents(
                event.isChildCalendar ? [event] : clientEvents(event._id), // get events with this ID
                clearEnd,
                newAllDay,
                dateDelta,
                durationDelta
    ...
    */
    ...
    MouseFollower.prototype = {
        ...
        stop: function (shouldRevert, callback) {
            ...
            if (this.isFollowing && !this.isAnimating) { // disallow more than one stop animation at a time
                ...
                $(document).off('mousemove', _this.mousemoveProxy);

                if (shouldRevert && revertDuration && !this.isHidden) { // do a revert animation?
                    var calendarMonthTop = window.localStorage.getItem("lastView") === 'month' ? $(".fc-view.fc-month-view").scrollTop() : 0, calendarMonthLeft = 0;
                    ...
                    _this.el.animate({
                        top: _this.top0 + calendarMonthTop,
                        left: _this.left0 - calendarMonthLeft
                    }, {
        ...
        mousemove: function (ev) {
            ...
            this.topDelta = ev.pageY - this.mouseY0 + calendarMonthTop;
            this.leftDelta = ev.pageX - this.mouseX0 - calendarMonthLeft;
    ...
    $.extend(Grid.prototype, {
        ...
        bindHandlers: function () {
            ...
            this.el.on('mousedown', function (ev) {
                if (
				!$(ev.target).is('.fc-more') && // not an an event element, or "more.." link .fc-event-container *
				!$(ev.target).closest('.fc-popover').length && // not on a popover (like the "more.." events one)
                $(ev.target).parents(".fc-draggable").length == 0 &&
                ($(ev.target).closest(".showActiveTitle").length == 0 || $(ev.target).closest(".fc-helper").length)
			) {
        ...
        dayMousedown: function (ev, ieVer) {
            var _this = this;
            var view = this.view;
    ...
    $.extend(DayGrid.prototype, {
        ...
        renderSegHtml: function (seg, disableResizing) {
            ...
            var backgroundColor = function (skinCss) {
                if (skinCss.indexOf("#EF9A9A") >= 0) //红色
                    return "background:#F44336";
                if (skinCss.indexOf("#CE93D8") >= 0) //紫色
                    return "background:#9C27B0";
                if (skinCss.indexOf("#BCAAA4") >= 0) //褐色
                    return "background:#795548";
                if (skinCss.indexOf("#FFCC80") >= 0) //橙色
                    return "background:#FF9800";
                if (skinCss.indexOf("#90CAF9") >= 0) //蓝色
                    return "background:#1E88E5";
                if (skinCss.indexOf("#A5D6A7") >= 0) //绿色
                    return "background:#4CAF50";
                if (skinCss.indexOf("#FFF59D") >= 0) //黄色
                    return "background:#FFEB3B";
                if (skinCss.indexOf("#E6E6E6") >= 0) //灰色
                    return "background:#B4B4B4";
            };

            // Only display a timed events time if it is the starting segment
            if (view.name == 'month') {
                if (!event.isAllDay && seg.isStart) {
                    var startTime = moment(event.startTime).format('HH:mm');
                    var endTime = moment(event.endTime).format('HH:mm');
                    if (moment(event.startTime).format('YYYY-MM-DD') != moment(event.endTime).format('YYYY-MM-DD'))
                        timeHtml = '<span class="fc-time" data-start="' + startTime + '" data-end="' + endTime + '">' + startTime + '</span>';
                    else {
                        timeHtml = '<span class="notAllDayMark" style="' + backgroundColor(skinCss) + '"></span> <span class="fc-time" data-start="' + startTime + '" data-end="' + endTime + '">' + startTime + '</span>';

                        hoverBgColor = backgroundColor(skinCss);
                        skinCss = skinCss.split(";")[skinCss.split(";").length-1];
                    }
                }
            }
            ...
            titleHtml =
			'<span class="fc-title">' +
				(event.isTask ? event.title : (htmlEscape(event.title || '') || '&nbsp;')) + // we always want one line of height
            '</span>';

            if (view.name !== 'month' && (!event.isAllDay && seg.isStart) && (moment(event.startTime).format('YYYY-MM-DD') != moment(event.endTime).format('YYYY-MM-DD'))) {
                titleHtml =
                    '<span class="fc-title">' +
				        (event.isTask ? event.title : (htmlEscape(event.title || '') || '&nbsp;')) + // we always want one line of height
                    '</span>';
            ...
            titleHtml = titleHtml.replace(_l('(全天)'), "<span class='fcTitleFont'>" + _l('(全天)') + "</span>");

            var bgClass = event.allDay ? " showActiveTitle" : " showActiveTitle notAllDayOver";
            if ((moment(event.end) - moment(event.start)) / (24 * 60 * 60 * 1000) > 1)
                bgClass = " showActiveTitle";

            return '<a class="' + classes.join(' ') + bgClass + '" data-hoverBgColor="' + hoverBgColor + '"' +
				(event.url ?
			...
			'>' +
				'<div class="fc-content">' +
					(((isRTL ? // put a natural space in between
    ...
    $.extend(DayGrid.prototype, {
		...
		if (segs[i].event.isTask) {
		    segContainer.find(".fc-title").eq(i).prepend('<span class="icon-calendartask" style="width:14px;display: inline-block;height:14px;margin: 1px 3px -2px 0;font-size: 16px;vertical-align: top;"> </span>');
		}

                if ($("#tb_OtherUserCalendar .addOtherUser").length > 0) {
                    segContainer.find(".fc-title").eq(i).prepend('<img src="' + segs[i].event.head + '" style="width:14px;margin-top: 1px; height:14px;margin-bottom:-2px;padding-right: 3px;">');
                }
            }
    ...
    $.extend(TimeGrid.prototype, {
        ...
        renderHelper: function (event, sourceSeg) {
            ...
            $($(".fc-helper-skeleton")).find(".fc-axis").css("width", "31px");

            this.helperEl.html(this.helperEl.html().replace("undefined", ""));
            this.helperEl.find(".removeClass").remove();
        },
    ...
    $.extend(TimeGrid.prototype, {
        ...
        renderSegHtml: function (seg, disableResizing) {
            ...
            //¼æÈÝÐ¡ÓÚ30·ÖÖÓµÄÊ±ºò£¬°´30·ÖÖÓÕ¼°ë¸ñºÍÊ±¼ä·ÅÔÚ±êÌâÀïÃæ
            var timeHtml = "", titleH = "", removeClass = "";
            if (event.startTime) {
                if ((moment(event.endTime) - moment(event.startTime)) < 60 * 60 * 1000) { //Ð¡ÓÚ60·ÖÖÓ
                    timeHtml = moment(event.startTime).format('HH:mm') + " - " + moment(event.endTime).format('HH:mm');
            ...
            var dataEnd = "";
            if (event.endTime) {
                dataEnd = moment(event.endTime).format('HH:mm');
            }
            return '<a class="' + classes.join(' ') + ' showActiveTitle"' +
			...
			'>' +
				'<div class="fc-content">' +
					(timeText ?
						...
						'>' +
							'<span class="' + removeClass + '">' + timeHtml + '</span>' +
						'</div>' :
						''
						) +
					(event.title ?
						'<div class="fc-title">' +
							titleH + htmlEscape(event.title) +
						'</div>' :
						''
						) +
				'</div>' +
