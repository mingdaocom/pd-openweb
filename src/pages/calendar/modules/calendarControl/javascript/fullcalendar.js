...
fc.datepickerLang = function (langCode, datepickerLangCode, options) {
  ...
  if ($.datepicker) {
    ...
    $.datepicker.regional[datepickerLangCode] = $.datepicker.regional[langCode] = // alias
      options;
          ...
          if (buttonName == 'title') {
            ...
            if (buttonClick) {
              ...
              button = $(
                ...
                '<button type="button" class="' + classes.join(' ') + '">' + innerHtml + '</button>',
              )
                .click(function () {
...
function EventManager(options) {
  ...
  function buildEvent(data, source) {
    ...
    var isReduceOneDay =
      moment(data.start).format('YYYY-MM-DD') === moment(data.end).add('d', -1).format('YYYY-MM-DD') &&
      data.start.substr(0, 10) !== data.end.substr(0, 10);
...
$.extend(DayGrid.prototype, {
  ...
  renderSegHtml: function (seg, disableResizing) {
    ...
    var backgroundColor = function (skinCss) {
      if (skinCss.indexOf('#EF9A9A') >= 0)
        //红色
        return 'background:#F44336';
      if (skinCss.indexOf('#CE93D8') >= 0)
        //紫色
        return 'background:#9C27B0';
      if (skinCss.indexOf('#BCAAA4') >= 0)
        //褐色
        return 'background:#795548';
      if (skinCss.indexOf('#FFCC80') >= 0)
        //橙色
    ...
    if (view.name == 'month') {
      if (!event.isAllDay && seg.isStart) {
        ...
        if (moment(event.startTime).format('YYYY-MM-DD') != moment(event.endTime).format('YYYY-MM-DD'))
          timeHtml =
            '<span class="fc-time" data-start="' + startTime + '" data-end="' + endTime + '">' + startTime + '</span>';
        else {
          timeHtml =
            '<span class="notAllDayMark" style="' +
            backgroundColor(skinCss) +
            '"></span> <span class="fc-time" data-start="' +
            startTime +
            '" data-end="' +
            endTime +
            '">' +
    ...
    if (
      ...
      !event.isAllDay &&
      seg.isStart &&
      moment(event.startTime).format('YYYY-MM-DD') != moment(event.endTime).format('YYYY-MM-DD')
    ) {
      titleHtml =
        '<span class="fc-title">' +
        (event.isTask ? event.title : htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
    ...
    if ((moment(event.end) - moment(event.start)) / (24 * 60 * 60 * 1000) > 1) bgClass = ' showActiveTitle';
...
$.extend(TimeGrid.prototype, {
  ...
  renderSegHtml: function (seg, disableResizing) {
    ...
    var timeHtml = '',
      ...
      removeClass = '';
    if (event.startTime) {
      if (moment(event.endTime) - moment(event.startTime) < 60 * 60 * 1000) {
        //Ð¡ÓÚ60·ÖÖÓ
        timeHtml = moment(event.startTime).format('HH:mm') + ' - ' + moment(event.endTime).format('HH:mm');
        removeClass = 'removeClass';
      } else timeHtml = htmlEscape(timeText);
    } else {
    ...
    return (
      ...
      (timeText
        ? '<div class="fc-time"' +
          ...
          '</div>'
        : '') +
      (event.title ? '<div class="fc-title">' + titleH + htmlEscape(event.title) + '</div>' : '') +
      '</div>' +
      '<div class="fc-bg"/>' +
      (isResizable ? '<div class="fc-resizer"/>' : '') +
      '</a>'
...
View.prototype = {
  ...
  render: function () {
    ...
    $(document).on('mousedown', this.documentMousedownProxy).on('dragstart', this.documentDragStartProxy); // jqui drag
  },
...
$.extend(AgendaView.prototype, {
  ...
  render: function (colCnt) {
    ...
    this.bottomRuleEl = $('<hr class="' + this.widgetHeaderClass + '"/>').appendTo(this.timeGrid.el); // inject it into the time-grid
