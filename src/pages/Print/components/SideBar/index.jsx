...
class SideNav extends React.Component {
  ...
  toggleApprovalCheckItem(index, childIndex = undefined) {
    ...
    const newApproval = approval.map((item, i) => {
      if (i === index) {
        approvalParentId = item.processId;
        item.checked = !item.checked;
              ...
              <div className="approvalItem1Con">
                ...
                {/* {item.child && item.child.length > 1 && (
                  <Icon
                    ...
                    className="Font18 expand Hand TxtCenter textTertiary"
                    onClick={() =>
                      this.setState({
    ...
    if (isReceiveControls) {
      handChange({
        receiveControls: receiveControls.map(it => {
          it.checked = controlState(it).visible && !receiveControlsCheckAll;

          if (isRelation(it)) {
            it.relationControls.map(a => {
              a.checked = controlState(a).visible && !receiveControlsCheckAll;
              return a;
            });
          }
                ...
                <div className="plate controlPlate">
                  <div className="caption">
                    ...
                    {plateItem.hasCheckAll && (
                      <span
                        className="Right Hand textTertiary chooseBtn"
