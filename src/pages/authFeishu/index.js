...
if (code) {
  if (checkLogin()) {
    ...
    ajax.post({
      url: __api_server__ + 'Login/FeishuAppLoginByApp',
      data: {
        code,
        state,
      },
  ...
  if (checkLogin()) {
    ...
    ajax.post({
      url: __api_server__ + 'Login/GetFeishuCorpInfoByApp',
      data: {
        projectId
      },
      async: true,
      succees: (result) => {
        ...
        const redirect_uri = encodeURIComponent(`${location.origin}/auth/feishu?url=${url ? encodeURIComponent(url) : ''}`);
