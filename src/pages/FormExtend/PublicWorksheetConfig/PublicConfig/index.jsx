...
class PublicConfig extends React.Component {
  ...
  constructor(props) {
    ...
    this.shareCardSetRef = React.createRef();
  }
  ...
  saveSetting = (cb = () => {}) => {
    ...
    updateSettings(newSettings, isSuccess => {
      if (isSuccess) {
        // 保存成功后，调用分享卡片的保存
        if (this.shareCardSetRef.current && activeTab === 2) {
        ...
        <div className="settingContent">
          ...
          {activeTab === 2 && (
            <WeChatEnhance
              ...
              handleUpdateExpandDatas={this.handleUpdateExpandDatas}
              handleSettingChanged={() => this.setState({ settingChanged: true })}
            />
          )}
          {activeTab === 3 && (
            <React.Fragment>
