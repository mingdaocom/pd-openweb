...
import { Hr, H1, H3, Absolute, Circle, CustomButton } from '../components/Basics';
import { themes, coverurls } from '../enum';

const Con = styled.div`padding: 0 24px;`;
const Close = styled.span`font-size: 18px; color: #9e9e9e; cursor: pointer;`;
const ColorCircle = styled(Circle)`
  ...
  background: ${({ bg }) => bg};
  :last-child { margin: 0 }
  .icon { position: absolute; left: 4px; top: 4px; }
...
const UploadBtn = styled(CustomButton)`
  margin-top: 20px;
  .icon { display: inline-block; position: relative; top: 1px; margin-right: 7px; font-size: 20px; }
`;

const ClearCover = styled(CustomButton)`
  :hover { background-color: #FEECEA; color: #F44336 }
`;
    ...
    >
      <Con>
        ...
        <div className="themePicker">
          {
            themes.map((theme, i) => <ColorCircle
              width="26"
              key={i}
              bg={theme.main}
              active={i === themeIndex}
        ...
        <div className="flexRow">
          <H3 className="flex" style={{ margin: '4px 0' }}>{ _l('封面图片') }</H3>
          <ClearCover height="28" borderRadius="14" onClick={() => (this.changeCover(''))}> { _l('清除图片') } </ClearCover>
        </div>
        <PictureSelect
          coverUrl={coverUrl}
          images={coverurls}
          onChange={(url) => (this.changeCover(url))}
        />
        <QiniuUpload
          className="Block"
          options={{
        ...
        >
          <UploadBtn
            ...
            style={{ margin: '10px 0' }}
          >
            { isUploading ? <i className="icon icon-loading_button rotate"></i> : <i className="icon icon-custom_insert_photo"></i> }
            { _l('上传自定义图片') }
          </UploadBtn>
        </QiniuUpload>
      </Con>
