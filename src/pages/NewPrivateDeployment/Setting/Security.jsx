...
const Register = props => {
  ...
  const [twoFactorAuthenticationPriorityType, setTwoFactorAuthenticationPriorityType] = useState(SysSettings.twoFactorAuthenticationPriorityType);
  const [enableTwoFactorAuthentication, setEnableTwoFactorAuthentication] = useState(SysSettings.enableTwoFactorAuthentication);
    ...
    <div className="privateCardWrap flexColumn">
      ...
      <div className="flexRow valignWrapper">
        <div className="flex flexColumn mRight60">
          ...
          <div className="Font13 Gray_9e mBottom8">{_l('此项为“登录二次验证”，开启后，除了基于账号密码的登录认证，还需要通过手机号或邮箱接收验证码进行二次认证，所以请确保组织成员的手机号、邮箱的合法性，同时确保系统内的短信服务、邮件服务已正常启用。')}</div>
        </div>
        <Switch
          checked={enableTwoFactorAuthentication}
          onClick={handleChangeEnableTwoFactorAuthentication}
        />
      </div>
    ...
    return (
      <div className="flexColumn">
        ...
        <div className="flexRow valignWrapper mBottom12">
          <div style={style} className="Gray_75">{_l('密码规则')}</div><div>{passwordRegex}</div>
        </div>
        <div className="flexRow valignWrapper mBottom12">
          <div style={style} className="Gray_75">{_l('提示说明')}</div><div>{passwordRegexTip}</div>
        </div>
        <div>
          <Button
            ghost
            type="primary"
            onClick={() => { setPasswordRuleDialogVisible(true) }}
          >
            {_l('设置')}
          </Button>
        </div>
