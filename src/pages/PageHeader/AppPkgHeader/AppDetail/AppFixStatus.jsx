...
export default class AppFixStatus extends Component {
  ...
  renderFooter() {
    ...
    if (fixed) {
      return (
        <Fragment>
          <Button type="ghostgray" onClick={this.handleCloseFix}>{_l('结束维护')}</Button>
          <Button type="primary" onClick={this.handleSave}>{_l('更新公告')}</Button>
        </Fragment>
      );
    } else {
      return (
    ...
    return (
      <Dialog {...options}>
        <div className="mBottom20 Gray_75 Font13">
          {fixed ? _l('若应用已更新维护完毕请结束维护使应用恢复正常，您也可以重新编辑公告内容。') : _l('应用设为维护状态后，普通成员无法使用应用，管理员可对应用进行更新维护。')}
        </div>
