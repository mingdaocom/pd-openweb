...
let AppInfo = class AppInfo extends Component {
  ...
  constructor(props) {
    ...
    // 此场景是搭建过程中的预览，不应弹出「应用详情/介绍」首入弹窗打扰预览。
    ...
    this.state = {
      ...
      isEditing: false,
      isShowAppIntroFirst: !_.includes(openedApps, appId) && !isRowInfo && !isAIPreview,
  ...
  componentDidUpdate(prevProps) {
    if (prevProps !== this.props) {
      ...
      if (compareProps(this.props.match.params, prevProps.match.params, ['appId'])) {
        this.getData(this.props);
      }

      if (data.id === getIds(prevProps).appId) {
        const isRowInfo = checkRecordInfo(this.props.location.pathname);
        const currentPcNaviStyle = isRowInfo ? 0 : data.pcNaviStyle;
        let appStatus = isRowInfo ? 0 : this.props.appStatus;

        if (this.props.appStatus === 300016 || _.get(this.props, 'appStatus.status') === 300016) {
          appStatus = 300016;
      ...
      if (
        this.ids.appId === getIds(prevProps).appId &&
        compareProps(this.props.match.params, prevProps.match.params, ['worksheetId'])
      ) {
        const { currentPcNaviStyle } = this.state.data;
        currentPcNaviStyle === 2 && this.checkIsFull(this.props.match.params.worksheetId);
      }
    }
  }
  ...
  getData = async (props = this.props) => {
    const requestId = ++this.appDetailRequestId;
    ...
    data.currentPcNaviStyle =
      checkRecordInfo(location.pathname) ||
      !_.find(pcNavList, {
        value: data.pcNaviStyle,
      })
        ? 0
        : data.pcNaviStyle;
      ...
      return (
        <Fragment>
            ...
            >
              ...
              >
                <div className="appIconWrap">
                  <SvgIcon url={iconUrl} fill={['black', 'light'].includes(themeType) ? iconColor : '#FFF'} size={24} />
                </div>
                <span
                  className={cx('appName', {
                    overflow_ellipsis: !isLeftAppStyle,
                    leftAppStyleAppName: isLeftAppStyle,
                  })}
                >
                  {showName}
                </span>
          ...
          {!(pcDisplay && !isAuthorityApp) && (fixed || isUpgrade || isMigrate) && (
            <div
              className={cx({
                appFixed: fixed || isMigrate,
                appUpgrade: isUpgrade,
              })}
            >
          ...
          {(!isHaveCharge(permissionType) ? description : false) && (isNormalApp || isMigrate) && (
            <Tooltip title={_l('应用说明')}>
              <Icon
                ...
                className="appIntroIcon Font16 pointer mLeft5"
                icon="info"
              />
            </Tooltip>
          )}
    ...
    if ([1, 3].includes(currentPcNaviStyle)) {
      ...
      return (
        <div className="appInfoWrap flexColumn pLeft10 pRight10 mBottom8">
          <div className="flexRow alignItemsCenter pTop10">
            ...
            {appStatus !== 300016 && !window.isPublicApp && ss && !md.global.Account.isPortal && !this.isAIPreview && (
            ...
            {appStatus !== 300016 && !(md.global.Account.isPortal || window.isPublicApp) && td && !this.isAIPreview && (
      ...
      return (
        <div className="appInfoWrap flexRow alignItemsCenter">
          {window.isPublicApp || !s || md.global.Account.isPortal || this.isAIPreview ? (
  ...
  render() {
    ...
    const canDebug = (debugRole || {}).canDebug || false; // 获取url参数

    const { s, tb, tr, ln } = getAppFeaturesVisible(); // 当导航方式为经典或卡片时URL的隐藏参数全写上后，顶部色块应该隐藏

    if (_.includes([0, 2], currentPcNaviStyle) && !s && !tb && !tr) return null; // loading 不展示导航
      ...
      <Fragment>
        ...
        <div
          className={cx(
            'appPkgHeaderWrap',
            {
              hide: [1, 3].includes(currentPcNaviStyle) && !ln,
            },
            themeType,
          )}
          style={{
          ...
          >
            <EditAppIntro
              ...
              description={
                isEditing
                  ? description
                  : (description ? getTranslateInfo(appId, null, appId).description || description : '') || ''
              }
              remark={data.shortDesc}
              permissionType={permissionType}
              isLock={isLock} // isEditing={!description && isAuthorityApp}
              isEditing={isEditing}
...
AppInfo = connect(mapStateToProps, mapDispatchToProps)(AppInfo);
