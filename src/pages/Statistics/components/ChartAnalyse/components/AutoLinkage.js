...
import { Modal, ConfigProvider, Button, Tooltip, Checkbox } from 'antd';
...
const getReportObject = (components, reports) => {
  return components.filter(c => [enumWidgetType.analysis, 'analysis'].includes(c.type)).map(c => {
...
export default class AutoLinkage extends Component {
  ...
  componentDidMount() {
    ...
    reportApi.listByPageId({
      appId: this.customPage.pageId
    }).then(data => {
      data = getReportObject(this.customPage.components, data).filter(c => c.reportId !== reportId).filter(c => c.worksheetId === worksheetInfo.worksheetId);
      const filterIds = autoLinkageChartObjectIds && autoLinkageChartObjectIds.filter(id => _.find(data, { objectId: id }));
      this.setState({
        components: data,
    ...
    return (
      <Con
        title={(
          <Fragment>
            {_l('选择联动筛选相关组件')}
             ({selectIds.length}/{components.length})
      ...
      >
        ...
        {components.filter(c => c.name.includes(searchValue)).map(c => (
          <div key={c.objectId} className="mBottom8">
            <Checkbox
              ...
              onChange={e => {
                const { checked } = e.target;
                this.setState({
                  selectIds: checked ? selectIds.concat(c.objectId) : selectIds.filter(objectId => c.objectId !== objectId)
                });
              }}
            >
              {c.name}
