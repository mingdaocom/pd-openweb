...
export default class PostMention extends React.Component {
  constructor(props) {
    ...
    if (props.inboxType === MSGTYPES.PostCommentMentionedUser
    ...
    } else if (props.inboxType === MSGTYPES.PostMentionedGroup ||
      props.inboxType === MSGTYPES.PostMentionedUser) {
      // 动态直接 提到
      this.state = {
  ...
  mergeTopicState(fromPost) {
    ...
    let item = null, comments, commentsCount;
    if (fromPost) {
      // 动态直接提到我，从Post中取
      item = inboxItem.Post;
  ...
  renderAccessory() {
    if (this.props.inboxType !== MSGTYPES.PostCommentMentionedUser
      ...
      case 0:
        ...
        return (
          <div className='mTop20 mBottom20 clearfix'>
            ...
            <div>
              <p className='ThemeColor3 mTop20' style={{ "font-size": "12px", "line-height": "20px" }}>{medalName}</p>
              <p className={{ "color": "#777", "font-size": "12px", "line-height": "20px" }}>
                {description}
              </p>
            </div>
          </div>
      ...
      case 1:
        ...
        return (
          <div>
            ...
            {linkThumb ?
              (
                <img className='lazy' src={linkThumb} />
              ) : null}
            {linkDesc ?
              (
                <div className="mTop5 Gray">
                  {linkDesc.toLowerCase()}
                </div>
              ) : null}
          </div>
        );
      ...
      case 7:
        ...
        return (
          <div className='messageDetailContainer'>
            <div className='Left logoContainer'><i className={'Font24 Gray_9' + className}></i></div>
            <div className='textContainer'>
              {message.substring(0, 25) + (message.length > 25 ? '...' : '')}
            </div>
            <div class='viewDetail'>
              <a target='_blank' href={fromLink} class='detailBtn boderRadAll_3'>{_l('查看详情')}</a>
            </div>
          </div>
        );
        break;
