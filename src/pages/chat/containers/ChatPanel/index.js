...
class ChatPanel extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (nextProps.visible !== this.props.visible && (!currentSession.value || currentSession.value !== this.props.currentSession.value)) {
      return false;
    }
    return true;
  }
  ...
  chatSessionItem(session) {
    ...
    this.ajax = ajax.chatSessionItem(session).done((result) => {
  ...
  render() {
    ...
    return (
      ...
      >
        {currentSessionList.map(item => <ChatPanelSession session={item} key={item.id} />)}
        {currentSession.iconType ? this.renderInbox() : undefined}
        {loading ? (
          <div className="ChatPanel ChatPanel-loading">
            {isError && !loading ? <div className="ChatPanel-error ThemeColor3" onClick={this.handleReset.bind(this)}>{_l('加载失败，点击重新加载。')}</div> : <LoadDiv size="middle" />}
          </div>
        ) : (
          undefined
        )}
