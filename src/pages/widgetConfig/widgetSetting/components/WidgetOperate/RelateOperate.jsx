...
export default function RelateOperate(props) {
  const { data, allControls, globalSheetControls, onChange, fromPortal } = props;
  ...
  const hasSet =
    filters.length ||
    searchcontrol ||
    (!searchcontrol && (searchfilters.length || clicksearch === '1')) ||
      ...
      {_.includes([0, 1], enumDefault2) && (
        <Fragment>
          <SheetViewWrap>
            ...
            />
            <div
              className="filterEditIcon tip-bottom-left"
              data-tip={_l('关联选择设置')}
              onClick={() => {
                if (!dataSource) {
                  alert(_l('请先选择工作表'), 3);
            ...
            >
              <i
                className={cx('icon-settings Font20 LineHeight34', {
                  ThemeColor3: hasSet,
                })}
              ></i>
            </div>
