...
function SelectConfig(props) {
  ...
  const renderContent = () => {
    const editProps = {
      ...props,
      data: configData,
      controls: _.get(window.subListSheetConfig[data.controlId], 'controls') || [],
      ...
      <SelectConfigWrap>
        <DisplayTabs>
          {TABS_DISPLAY.map(item => {
            ...
            if (item.value === 3 && data.enumDefault === 1) return null;
            if (_.includes([2, 3], item.value) && showtype === '3') return null;
            return (
              <div
