...
export default function DestEdit(props) {
  ...
  const isPgDest = isPostgresqlDestinationType(_.get(node, ['nodeConfig', 'config', 'dsType']));
    ...
    <React.Fragment>
      ...
      <FieldMappingList
        ...
        setFieldsMapping={(mapping, isOnChange) => {
          ...
          let nextMapping = data;

          if (_.get(node, ['nodeConfig', 'config', 'createTable']) && isPgDest) {
            nextMapping = data.map(row => {
              const d = row.destField;
              if (!d) return row;
              return {
                ...row,
                destField: {
                  ...d,
                  name: lowercaseIfString(d.name),
                  alias: lowercaseIfString(d.alias),
                },
              };
            });
          }
