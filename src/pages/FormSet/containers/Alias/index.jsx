...
export default function Alias(props) {
  ...
  return (
    <React.Fragment>
      ...
      {showAliasDialog && (
        <AliasDialog
          data={(_.get(worksheetInfo, 'template.controls') || []).filter(it => !ALL_SYS.includes(it.controlId))}
          worksheetId={worksheetId}
          appId={appId}
          onClose={(isUpdate, data) => {
            setState({ showAliasDialog: false });
            if (isUpdate) {
              onChange({ ...worksheetInfo, template: { ...(worksheetInfo || {}).template, controls: data } });
            }
          }}
        />
      )}
