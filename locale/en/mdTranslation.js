var translations = {
  ...
  "完善组织信息": "Complete Org Information",
  "%0 W次": "%0 W times",
  ...
  "万行": "10K Rows",
  "%0 万行 / %1 万行": "%0 10K rows / %1 10K rows",
  "上限 %0 万行": "Limit %0 10K rows",
  ...
  "主题和图表配色联动，实现风格的一致性": "Theme and chart colors linked for consistent style.",
  ...
  "流程中有其他PBP、子流程、循环流程、代办、延时节点时，输出参数映射无效": "Invalid output parameter mapping when there are other PBPs, subflows, loop, todo, and delay nodes in the process",
  "参数映射": "Mapping",
  "开发调试": "Development",
  ...
  "仅对主流程进行测试执行。引用的子流程、PBP、审批流程将跳过执行，可单独前往测试这些流程。": "Only execute tests on the main process. Referenced subflows, PBP, and approval processes will be skipped during execution and can be tested separately.",
  ...
  "原始语言指搭建应用时使用的语言，eg:搭建应用时的文本语言(字段名称、标题等)为法语，则可以选择法语为您的原始语言。": "Original language refers to the language when developing an App. Eg: if you edit text (field names, titles, etc.) in French when building an App, you can select French as your original language.",
  "弹层": "Popup",
  ...
  "给AI模型设置提示词，可引用其他节点动态值。": "Set prompt for the AI model, you can reference dynamic values from other nodes.",
  ...
  "选择一个子表或关联表，系统将于每个节点完成后，自动同步本流程的各人工节点的操作明细至该表": "Select a subform or related table. The system will automatically synchronize the operation details of each manual node in this workflow to that table after each node is completed.",
  "选择子表或关联记录": "Select subforms or associated records",
  ...
  "请选择子表中的字段": "Please select fields from the subform",
  ...
  "在用户输入时字段的显示名称。如：请输入手机号": "The field's display name when user input is needed. For example: Please enter your mobile number",
  ...
  "只勾选微信登录后首次扫码后需要输入手机号与微信绑定，后续可单独微信扫码快速登录。": "WeChat scan for quick login if only select to enter phone number and bind with WeChat after first scan.",
  ...
  "“%0” 导出文件已准备完成，文件下载链接将于24小时失效，请尽快下载": "The export file of “%0” is ready. Download link will expire after 24 hours, please download ASAP.",
  ...
  "确认删除所选择的工作地点？": "Are you sure to delete the selected workspace?",
  ...
  "自定义签名仅支持国内号码，国际号码验证短信固定使用 【明道云】 短信签名": "Custom signatures are only supported for domestic numbers. For international numbers, the verification SMS will use the fixed signature of 【Mingdao】.",
  ...
  "可将指定应用内的所有工作表数据存储到专属数据库中，免受系统默认数据库的影响，适用于隔离等场景，系统默认支持最多创建%0个可用专属数据库实例；管理员创建应用时，可选择专属数据库。": "Store all worksheet data in a specified app into a dedicated database, isolated from the default system database, ideal for isolation scenarios. System supports up to %0 dedicated database instances; admins can select a dedicated database when creating an app.",
  ...
  "会签（只需一名审批人通过，否决需全员否决）": "Co-signing (Approved when one approves, declined when all decline)",
  "或签（一名审批人通过或否决即可）": "Or sign (one approver's approval or rejection is sufficient)",
  "通过 %0": "By %0",
  "通过 %0 的 %1": "%1 through %0",
  "通过审批": "Approved",
  "通过申请并加签": "Approved and add signature",
  "通过": "Approved",
  "启用后，通过点击表单提交按钮创建的记录，在触发工作流后会立即开始执行（无需系统默认的5s延时等待）。当执行完成或等待时会同时刷新前端数据。": "After enabling, created records will trigger workflow execution immediately (less then 5-second). Front-end data refreshes post-execution or during wait.",
  ...
  "可以在宣传栏中添加图片链接，展示组织重要信息": "Add image links in the banner section display important information.",
  ...
  "个人认证 - 明道云HAP": "Personal Certification - Mingdao HAP",
