var translations = {
  ...
  "中止流程，或继续执行查找结果分支": "Terminate the process or proceed with the searched result branch",
  ...
  "用于系统中发送邮件、短信等计费服务项目自动扣费。为避免系统功能不可用，请保障账户余额充足。【点击查看详细说明】": "Used for automatic billing of system services. Ensure sufficient balance to avoid service disruption.【Click to view detailed】",
  ...
  "试用、免费版需组织完成身份认证后可充值余额；自定义短信签名需组织完成企业身份认证（注意：非个人身份）": "Trial and free versions require identity verification for balance recharge. Custom SMS signatures require corporate identity verification (note: not personal identity).",
  ...
  "添加动作": "Add",
  "定义输出数据对象的参数，参数名称只能是英文、数字和下划线。参数名称和说明需要含义清晰以便 AI 识别。": "Define output data parameters with names consisting of letters, numbers, and underscores. Ensure the name and description are clear for AI.",
  "参数名称": "Name",
  "参数名": "Name",
  "字段名/参数名": "Field Name/Parameter Name",
  "指定参数名，可作为查询字符串附加在自定义页面链接后。在加载页面时可动态获取参数值用于自定义页面的筛选条件。": "Specify the parameter name, which can be appended as a query string to the custom page link. The parameter value can be dynamically retrieved when loading the page and used as a filter condition for the custom page.",
  ...
  "耗时": "Duration",
  "整体耗时": "Time spent",
  "内不发送同类错误通知": "Do not send similar error notifications",
  "填写%03025": "Fill in",
  "转交他人填写": "Forward",
  ...
  "验证节点负责人的记录查看权限": "Verify node owner's record view  permission",
  ...
  "用于工作流短信节点或外部门户短信邀请，费用0.1元/条": "Used for workflow SMS nodes or external portal SMS invitations, cost 0.1 CNY/message",
  ...
  "允许扫描条形码": "Enable barcode scanning",
  ...
  "流程中有其他PBP、子流程、循环流程、代办、延时节点时，输出参数映射无效": "When other PBP, subflow, loop, to - do or delay nodes exist in the process, output parameter mapping is invalid",
  "选择字段": "Select fields",
  ...
  "获取后直接提交表单": "Submit the form after scan",
  "在显示表单前先获取输入": "Scan before displaying the form",
  ...
  "拍摄视频": "Record video",
  "请向我描述您想要实现的字段功能，我将为你生成代码。你可以在左侧实时预览代码效果，也可以向我追加提问修改代码或者提供报错信息让我协助排查问题。": "Please describe the field functionality you want to achieve, and I will generate the code for you. You can preview the code effect in real time on the left, or ask me additional questions to modify the code or provide error messages for troubleshooting assistance.",
  "定位": "Positioning",
  "签名": "Signature",
  "单选": "Single-select",
  "多选": "Multi-select",
  ...
  "应用项名称": "App Item Name",
  "权限方式：分发所有应用项，80：可查看、编辑、删除所有记录，60：可查看所有记录，但只能编辑、删除自己拥有的记录，30：可查看加入的，只能编辑、删除自己拥有的记录，20：对所有记录只有查看权限，0：分发有权限应用项": "Permission Type: Distribute all app items, 80: Can view, edit, and delete all records, 60: Can view all records, but can only edit and delete records owned by oneself, 30: Can view joined records, but can only edit and delete records owned by oneself, 20: Only has view permissions for all records, 0: Distribute application items with permissions",
  ...
  "分发所有应用项（简单）": "Distribute all app items (simple)",
  "分发有选择的应用项（高级）": "Distribute selected app items (advanced)",
  "应用项ID": "App Item ID",
  "应用项": "Items",
  ...
  "更新于": " Updated",
  "刚刚": "Just now",
  "表格视图中的记录在移动端以卡片的形式呈现，为移动端设置记录的显示布局，以便于移动端的数据浏览和操作。": "The card is a efficient way on mobile devices, adjust the card style to fit you better.",
  ...
  "输入参数进行测试": "Enter parameters for testing",
  ...
  "文字（单元格背景色）": "Text (cell background color)",
  ...
  "可将指定应用内的所有工作表数据存储到专属数据库中，免受系统默认数据库的影响，适用于隔离等场景，系统默认支持最多创建%0个可用专属数据库实例；管理员创建应用时，可选择专属数据库。": "Store all worksheet data in a specified app into a proprietary database, isolated from the default system database, ideal for isolation scenarios. System supports up to %0 proprietary database instances; admins can select a proprietary database when creating an app.",
  ...
  "当前有尚未保存的更改，您在离开当前页面前是否需要保存这些更改。": "There are unsaved changes at present. Save them before your leave?",
  "开关控制登录页面是否显示 7 天免登录的选项": "The switch controls whether the \"7-day free login\" option is displayed on the login page",
  "登录页面是否显示 7 天免登录的选项": "Display the option of 7-day free login in login page",
  "7天免登录": "Log in free for 7 days",
  "组织成员查看": "Organization members' view",
  "外部用户查看": "External user's ’view",
  ...
  "切换后，应用成员回复已有讨论，回复内容归属于内部讨论，外部用户不可查看且不能回复": "After the switch, application members reply to existing comments, the reply belongs to the internal comments which external users can not view or reply",
  ...
  "验证码每条%0，将自动从企业账户扣除。": "Each verification code costs %0, deducted from the enterprise account automatically.",
  ...
  "外部用户注册开始/停止时间": "Limit external user registration start/stop time",
  "通过微信扫码登录时，需先关注服务号": "When logging in via WeChat QR code, follow the service account first",
  ...
  "搜索委托人": "Search Delegate",
  "发起委托": "Initiate Delegation",
  "添加委托成功": "Delegation Successful",
  ...
  "委托人": "Delegate",
  "请先选择委托人": "Please select the delegate first",
  ...
  "批量修改成功，保存后生效": "Batch update successful,save to apply changes",
  "批量编辑": "Batch Edit",
  ...
  "商户号配置指引": "Merchant ID Configuration Guide",
  ...
  "百慕大": "Bermuda",
  "帕劳": "Palau",
  "当未设自定义排序，或自定义排序后同顺序下有多条记录时，将追加使用默认排序。默认排序只能使用创建时间或最近更新时间，来确保数据最终有严格顺序。": "When no custom sorting is set, or multiple records share the same order after custom sorting, default sorting will be applied. Default sorting can only use creation or last update time to ensure a strict order.",
  ...
  "打开记录时显示审批流转详情": "Show approval flow details when opening a record",
  ...
  "浮窗打开": "View in pop-up",
  "导入升级": "App Upgrade",
  "关闭后，全组织下的自定义按钮、审批配置了登录密码验证的地方必须每次验证密码后方可继续操作": "After disabling, custom buttons and approval setups in the org that require login password verification must be verified each time",
  "创建应用": "Create App",
  ...
  "人员加入组织需要填写的信息": "Information needs to be filled when joining",
  ...
  "文本索引": "Text Index",
  "当月同步任务算力剩余2%时自动购买当月额度包，费用10元/10,000行": "Auto-purchase monthly package when the remaining computing power of sync tasks for current month is 2%, cost CNY 10 /10,000 rows",
  ...
  "当月工作流执行数剩余2%时自动购买当月额度包，费用10元/1,000次": "Auto-purchase monthly package when the remaining workflow execution times for current month is 2%, cost CNY 10 /1,000 times",
  ...
  "当前有尚未保存的更改，你在离开页面前是否需要保存这些更改？": "You have unsaved changes. Do you want to save them before leaving?",
  "当前有尚未保存的修改，你在离开页面前是否需要保存这些修改？": "You have unsaved changes. Do you want to save them before leaving?",
  ...
  "等我审批...": "To approve...",
  "等我填写...": "To fill in...",
  "按照所选视图配置的显示字段发送，如果通知人被分发了此视图，可以直接按权限编辑记录、执行自定义动作": "Send the configured view fields; if the recipient has access, they can edit records or perform actions as permitted",
  ...
  "刷新频率过快": "The refresh rate is too high",
  "刷新频率": "Refresh Rate",
  ...
  "四舍五入": "round off",
  "向下舍入": "round down",
  "参与计算的字段值为空时，视为 0": "If calculation - involved field values are empty, treat as 0",
  "当工作表记录超过10万行时，为保证工作表性能，修改配置后将不再刷新历史数据。如果仍需更新历史数据，请手动进行刷新。": "When worksheet records exceed 100,000 rows, to ensure performance, historical data won't refresh post - config change. Manual refresh is needed for updates.",
