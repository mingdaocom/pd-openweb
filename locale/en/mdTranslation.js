var translations = {
  ...
  "2.离线模式支持文本、数值、金额、邮箱、日期、时间、电话、附件、检查项、富文本、证件、定位、单选（静态数据）、多选（静态数据）、签名字段数据的录入。": "2. The offline mode supports text, number, currency, emails, date, time, phone number, attachment, check items, rich text, document, position, single-choice (static data), multiple-choice (static data), and signature field data.",
  ...
  "开通后完成交易7天内的订单支持申请退款，目前仅付款人可申请退款": "When enabled, orders that are completed within 7 days support refund requests. Currently, only the payer can request a refund",
  "允许退款": "Allow Refunds",
  ...
  "未启用时，默认输入中国号码，国际号码需要手动输入国家/地区代码。启用后，可选择国家/地区代码。": "When not setting, default input Chinese number; international numbers require manual entry with country code.",
  ...
  "在付费版下，您可以通过组织的二级域名登录页面，通过集成LDAP账号登录，实现统一身份认证管理 （确保员工的账号已与邮箱绑定，系统通过邮箱进行映射）": "In the paid version, you can log in via secondary domain and integrated LDAP account for unified authentication (Please ensure the employee's account is linked to their email for system mapping)",
  ...
  "手机": "Mobile",
  "电话": "Telephone",
  "试用、免费版需组织完成身份认证后可充值余额；自定义短信签名需组织完成企业身份认证（注意：非个人身份）": "Trial/Free Edition require Org verification for balance top-up; custom SMS signature requires corporate verification (not individual).",
  ...
  "您尚有未退出的组织，请先至 个人中心-我的组织 退出所有组织，方可注销！": "If you have organizations not logged out, go to Personal Account - My Org to log out before logging out !",
  "您尚有未退出的组织，请先至 个人中心-我的组织 退出所有组织，方可注销": "If you have organizations not logged out, go to Personal Account - My Org to log out before logging out",
  ...
  "用于配置鉴权时使用，设置为隐藏后，查看时该参数值将以掩码代替": "Used for configuring authentication, the value of this parameter will be replaced by mask when setting to hidden",
  ...
  "宣传栏": "Banner",
  "将复制目标应用的应用结构、流程和角色。应用下的数据和成员不会被复制": "Only the structure, workflow and role will be copied. The row data within the original app will not be copied.",
  "删除应用": "Delete",
  "注意：应用下所有配置与数据(包括聚合表)将被删除，删除后可在工作台首页回收站恢复。": "Warning: All settings and data (including aggregate tables) within the app will be deleted. You can RESTORE them from the Recycle Bin.",
  ...
  "3.离线录入时，表单的业务规则、字段验证和权限控制都不生效。数据暂存本地，网络恢复后需手动提交，确保数据结构完整性。": "3. The offline mode does not support the entry of the business rules, field validations, and permission controls of the form. Data is temporarily stored locally, and must be manually submitted after the network is restored to ensure data structure integrity.",
  ...
  "1.支持App在无网络环境下完成数据录入，恢复网络后手动同步至服务器，确保数据的完整性和可靠性。": "1. Supports data entry in the App in offline mode, and manually syncs to the server after the network is restored, ensuring data integrity and reliability.",
  "1.导入的数据%0不会触发工作流%1": "1. The imported data %0 will not trigger workflow %1",
  "APP离线提交": "APP Offline Submission",
  "离线提交": "Offline Submission",
  "暂无离线提交": "No offline submission available",
  ...
  "如需导出全部数据，可联系顾问或通过": "If you need to export all data, please contact your consultant or put forward",
  "导入后不允许修改应用配置": "Changing app configs is not allowed after exporting",
  ...
  "展示其他字段对当前字段的关键引用，主要包括数据生成、逻辑计算、数据联动、数据限制等可能影响数据完整性或正确性的引用": "Display key references from other fields to the current field, including data generation, logic calculation, data linkage, and data constraints that may affect data integrity or accuracy",
  ...
  "开启后，当月剩余执行额度为2%时，自动购买 100元/10万行 的单月包，从账户余额中扣款。": "After opening, when the remaining execution limit for the month is 2%, it will automatically purchase a monthly package of CNY 100 /100K rows and deduct it from the account balance.",
