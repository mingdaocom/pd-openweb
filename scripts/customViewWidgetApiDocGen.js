...
function parseApiControllersFromWidgetFunctions() {
  const widgetFunctionsPath = path.join(ROOT_PATH, 'src/pages/worksheet/views/CustomWidgetView/widgetFunctions.js');
  ...
  return controllers.map(({ controller, ajaxImportName }) => {
    return {
      ...
      relativePath: importMap[ajaxImportName] || '',
    };
  });
}
...
function parseJSFile(filePath) {
  ...
  print.info(`共找到 ${methods.length} 个方法`);
  return methods;
}
...
function main() {
  ...
  print.info(`自动解析到的 API 控制器: ${apiControllers.map(c => c.name).join(', ')}`);

  const needParseApiControllers = apiControllers.map(controller => {
    return {
      filePath: path.join(ROOT_PATH, controller.relativePath) + '.js',
  ...
  const allMarkdown =
    ...
    '\n\n' +
    markdownList.join('\n---\n\n');
  fs.writeFileSync(path.join(OUTPUT_DIR, 'api-doc.md'), allMarkdown);
  fs.writeFileSync(path.join(OUTPUT_DIR, 'api-doc.json'), JSON.stringify(controllerData));

  const controllerTypeList = generateControllerType(controllerData);
  fs.writeFileSync(path.join(OUTPUT_DIR, 'api-doc.d.ts'), controllerTypeList);

  const mdyeApiJs = generateMdyeApiJs(controllerData);
  fs.writeFileSync(path.join(OUTPUT_DIR, 'mdye-api.js'), mdyeApiJs);

  writeAiDocs(controllerData);
  print.success(`API 文档已生成到: ${OUTPUT_DIR}`);
}
...
module.exports = {
  main,
  parseApiControllersFromWidgetFunctions,
};
